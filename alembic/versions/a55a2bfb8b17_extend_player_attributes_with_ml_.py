...
def upgrade() -> None:
    ...
    if 'player_attributes' not in inspector.get_table_names():
        logger.error("player_attributes table not found! Cannot proceed with migration.")
        raise Exception("PlayerAttributes table does not exist")
    
    # Get existing columns to avoid duplicate additions
    existing_columns = {col['name'] for col in inspector.get_columns('player_attributes')}
    ...
    with op.batch_alter_table('player_attributes', schema=None) as batch_op:
        ...
        if 'xg_per_90' not in existing_columns:
            ...
            batch_op.add_column(sa.Column(
                'xg_per_90', sa.Float(), nullable=True,
                comment='Expected goals per 90 minutes'
            ))
            batch_op.add_column(sa.Column(
                'xa_per_90', sa.Float(), nullable=True,
                comment='Expected assists per 90 minutes'
            ))
            batch_op.add_column(sa.Column(
                'xgi_per_90', sa.Float(), nullable=True,
        ...
        if 'form_3_games' not in existing_columns:
            ...
            batch_op.add_column(sa.Column(
                'form_3_games', sa.Float(), nullable=True,
                comment='Average FPL points over last 3 games'
            ))
            batch_op.add_column(sa.Column(
                'form_5_games', sa.Float(), nullable=True,
                comment='Average FPL points over last 5 games'
            ))
            batch_op.add_column(sa.Column(
                'form_10_games', sa.Float(), nullable=True,
                comment='Average FPL points over last 10 games'
            ))
            batch_op.add_column(sa.Column(
                'momentum', sa.Float(), nullable=True,
        ...
        if 'next_3_fixture_difficulty' not in existing_columns:
            ...
            batch_op.add_column(sa.Column(
                'next_3_fixture_difficulty', sa.Float(), nullable=True,
                comment='Average difficulty rating for next 3 fixtures (1-5 scale)'
            ))
            batch_op.add_column(sa.Column(
                'next_5_fixture_difficulty', sa.Float(), nullable=True,
        ...
        if 'is_penalty_taker' not in existing_columns:
            ...
            batch_op.add_column(sa.Column(
                'is_penalty_taker', sa.Boolean(), nullable=True,
                comment='Whether player is primary penalty taker'
            ))
            batch_op.add_column(sa.Column(
                'is_free_kick_taker', sa.Boolean(), nullable=True,
                comment='Whether player is primary free kick taker'
            ))
            batch_op.add_column(sa.Column(
                'is_corner_taker', sa.Boolean(), nullable=True,
                comment='Whether player is primary corner taker'
            ))
            batch_op.add_column(sa.Column(
                'role_confidence', sa.Float(), nullable=True,
        ...
        if 'shots_per_90' not in existing_columns:
            ...
            batch_op.add_column(sa.Column(
                'shots_per_90', sa.Float(), nullable=True,
                comment='Total shots per 90 minutes'
            ))
            batch_op.add_column(sa.Column(
                'key_passes_per_90', sa.Float(), nullable=True,
                comment='Key passes (passes leading to shots) per 90 minutes'
            ))
            batch_op.add_column(sa.Column(
                'tackles_per_90', sa.Float(), nullable=True,
                comment='Successful tackles per 90 minutes'
            ))
            batch_op.add_column(sa.Column(
                'interceptions_per_90', sa.Float(), nullable=True,
                comment='Interceptions per 90 minutes'
            ))
            batch_op.add_column(sa.Column(
                'clearances_per_90', sa.Float(), nullable=True,
                comment='Clearances per 90 minutes'
            ))
        else:
            logger.info("Advanced performance statistics columns already exist, skipping...")
    ...
    existing_indexes = {idx['name'] for idx in inspector.get_indexes('player_attributes')}
    ...
    logger.info(f"Migration completed successfully! Created {created_count} new indexes.")
...
def downgrade() -> None:
    ...
    inspector = sa.inspect(connection)
    if 'player_attributes' not in inspector.get_table_names():
        logger.warning("player_attributes table not found! Migration already rolled back or table doesn't exist.")
        return
    ...
    logger.info(f"Found {len(existing_columns)} existing columns and {len(existing_indexes)} existing indexes")
    ...
    logger.info(f"Rollback completed successfully! Dropped {dropped_columns} columns and {dropped_indexes} indexes.")
