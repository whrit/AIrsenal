...
from airsenal.framework.schema import get_connection_string, session_scope
...
class MigrationValidator:
    ...
    def _check_current_schema_consistency(self) -> bool:
        ...
        try:
            ...
            required_columns = [
                'id', 'player_id', 'season', 'gameweek', 'price', 
                'team', 'position', 'chance_of_playing_next_round'
            ]
        ...
        try:
            with self.engine.connect() as conn:
                ...
                seasons_result = conn.execute(text(
                    "SELECT COUNT(DISTINCT season) FROM player_attributes"
                ))
    ...
    def _check_new_columns_exist(self) -> bool:
        ...
        expected_columns = [
            ...
            'form_3_games', 'form_5_games', 'form_10_games', 'momentum',
            'next_3_fixture_difficulty', 'next_5_fixture_difficulty',
            'is_penalty_taker', 'is_free_kick_taker', 'is_corner_taker', 'role_confidence',
    ...
    def _check_new_indexes_exist(self) -> bool:
        ...
        expected_indexes = [
            'ix_form_3_games', 'ix_form_5_games', 'ix_xg_per_90', 'ix_xgi_per_90',
            'ix_next_3_fixture_difficulty', 'ix_position_penalty_taker', 'ix_momentum'
        ]
        ...
        try:
            ...
            for col_name, expected_type_variants in expected_types.items():
                if col_name in column_types:
                    ...
                    type_match = any(exp_type in actual_type for exp_type in expected_type_variants)
                    
                    if type_match:
                        print(f"✅ {col_name}: {actual_type}")
                    else:
                        print(f"❌ {col_name}: Expected {expected_type_variants}, got {actual_type}")
                        return False
        ...
        try:
            with self.engine.connect() as conn:
                ...
                if post_count == valid_records:
                    ...
                    print(f"❌ Data integrity: {post_count - valid_records} corrupted records")
                    return False
        except Exception as e:
    ...
    def rollback_safety_check(self) -> bool:
        ...
        try:
            ...
            columns_to_check = [
                'xg_per_90', 'form_3_games', 'is_penalty_taker'  # Sample columns
            ]
