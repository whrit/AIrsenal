...
class MockAdaptiveModel(AdaptivePlayerModel):
    ...
    def predict(
        self, features: np.ndarray, player_data: dict[str, Any]
    ) -> dict[str, float]:
        # Simple linear prediction
        if len(features) >= 10:
            prediction = (
                np.dot(self.state["weights"], features[:10]) + self.state["bias"]
            )
        else:
    ...
    def update_model(
        self, new_data: list[dict[str, Any]], learning_rate: float = 0.01
    ) -> None:
        ...
        for data_point in new_data:
            if "actual_points" in data_point and "features" in data_point:
                ...
                if len(features) == 10:
                    error = data_point["actual_points"] - np.dot(
                        self.state["weights"], features
                    )
...
class MockAvailabilityPredictor(AvailabilityPredictor):
    ...
    def predict_availability(
        self, player_data: dict[str, Any], gameweek: int
    ) -> dict[str, float]:
...
class MockFormCalculator(FormCalculator):
    ...
    def calculate_form(
        self, recent_performances: list[dict[str, Any]], window_size: int = 5
    ) -> dict[str, float]:
        ...
        [p.get("minutes", 0) for p in recent]
    ...
    def calculate_weighted_form(
        self, performances: list[dict[str, Any]], weights: list[float] | None = None
    ) -> float:
        if not performances:
            return 0.0
        ...
        weighted_sum = sum(p * w for p, w in zip(points, weights, strict=False))
...
class BaseModelsBenchmarks:
        ...
        return [
            {
                ...
                "injury_history": [{"type": "minor", "duration": 2}]
    ...
    def benchmark_adaptive_model_operations(self):
        ...
        with self.runner.benchmark_context("adaptive_model_predict_single"):
            for player_data in self.test_player_data[:50]:
                ...
                self.adaptive_model.predict(features, player_data)
        ...
        update_data = [
            {"features": player["features"], "actual_points": np.random.uniform(0, 15)}
    ...
    def benchmark_availability_prediction(self):
        ...
        with self.runner.benchmark_context("availability_predict_single"):
            for player_data in self.test_player_data[:50]:
                self.availability_predictor.predict_availability(
        ...
        with self.runner.benchmark_context("availability_update_injury"):
            for i, player_data in enumerate(self.test_player_data[:20]):
                injury_data = {"type": "minor", "duration": 2, "probability": 0.3}
    ...
    def benchmark_form_calculation(self):
        ...
        with self.runner.benchmark_context("form_calculate_single"):
            for _ in range(50):
                self.form_calculator.calculate_form(
                    self.test_performances[:5], window_size=5
                )
        ...
        for window_size in [3, 5, 10]:
            with self.runner.benchmark_context(f"form_calculate_window_{window_size}"):
                for _ in range(20):
                    self.form_calculator.calculate_form(
                        self.test_performances, window_size=window_size
                    )

        # Weighted form calculation
        with self.runner.benchmark_context("form_calculate_weighted"):
            for _ in range(50):
                self.form_calculator.calculate_weighted_form(
