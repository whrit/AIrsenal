    ...
    features_to_register = [
        {
            ...
            "computation_logic": {"type": "rolling", "metric": "minutes", "window": 10, "agg": "mean"}
        },
        {
            "name": "rolling_points_volatility",
            "feature_type": "player",
            "description": "5-game rolling standard deviation of points",
            "computation_logic": {"type": "rolling", "metric": "points", "window": 5, "agg": "std"}
        },
        {
            "name": "minutes_consistency",
            "feature_type": "player",
            "description": "How consistent player's minutes are (lower = more consistent)",
            "computation_logic": {"type": "rolling", "metric": "minutes", "window": 8, "agg": "std"}
        },
        {
            "name": "hot_streak",
            "feature_type": "player",
            "description": "Exponentially weighted recent form",
            "computation_logic": {"type": "form", "metric": "points", "decay_factor": 0.8}
        }
    ]
...
def example_4_prediction_integration():
    ...
    try:
        ...
        if prediction_features and len(prediction_features) > 0:
            ...
            for feature_name in first_player.keys():
    ...
    fpl_features = [
        {
            ...
            "computation_logic": {
                "type": "composite",
                "components": ["rolling_points_5", "fixture_difficulty", "home_advantage"]
            }
        },
        {
...
def main():
    ...
    try:
        ...
        print("="*60)
        print("\nNext steps:")
        print("1. Run the feature store tests: pytest airsenal/tests/test_feature_store.py")
        print("2. Migrate your historical data: python -c 'from examples.feature_store_usage import migrate_historical_data; migrate_historical_data()'")
