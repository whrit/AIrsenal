...
def example_basic_redis_operations():
    ...
    # Store DataFrame
    df = pd.DataFrame(
        {"player_id": [1, 2, 3], "goals": [2, 1, 0], "minutes": [90, 85, 60]}
    )
...
def example_prediction_caching():
    ...
    cached_predictions = redis_cache.get_prediction_cache(
        player_id=player_id, gameweek=gameweek, season=season, model_version="v2.1"
    )
...
def example_cache_invalidation():
    ...
    print(
        f"Data invalidated: pred={cached_pred is None}, feature={cached_feature is None}"
    )
