...
def predict_player_points(
    player_id: int, gameweek: int, features: dict[str, Any]
) -> float:
    """
...
def example_error_handling():
    ...
    set_context(operation="error_example", player_id=123, gameweek=5)

    try:
        logger.info("Starting operation that might fail")

        # Simulate an error
        msg = "Example error for demonstration"
...
def example_batch_processing():
    ...
    set_context(
        operation="batch_prediction", gameweek=gameweek, batch_size=len(player_ids)
    )
    ...
    for i, player_id in enumerate(player_ids):
        ...
        try:
            ...
            predict_player_points(
                player_id, gameweek, {"form": 0.75, "fixture_difficulty": 3}
            )
...
def example_complex_workflow():
    ...
    set_context(workflow="gameweek_processing", gameweek=5, season="2324")
