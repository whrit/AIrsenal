...
class CICDValidator:
    ...
    def validate_workflow_files(self):
        ...
        for workflow_file, description in required_workflows.items():
            ...
            if not workflow_path.exists():
                msg = f"Missing workflow: {workflow_file} ({description})"
            ...
            try:
                ...
                if "on" not in workflow_config:
                    msg = f"{workflow_file}: Missing trigger configuration ('on' field)"
                    raise ValueError(msg)

                if "jobs" not in workflow_config:
                    msg = f"{workflow_file}: Missing jobs configuration"
                    raise ValueError(msg)

                self.info.append(f"  ✓ {workflow_file}: Valid structure")

            except yaml.YAMLError as e:
                msg = f"{workflow_file}: Invalid YAML syntax - {e}"
                raise ValueError(msg)
    ...
    def validate_dependencies(self):
        ...
        if not pyproject_path.exists():
            msg = "Missing pyproject.toml file"
            raise FileNotFoundError(msg)
        ...
        for dep, description in required_deps.items():
            if dep not in dependency_strings.lower():
                self.warnings.append(
                    f"Missing recommended dependency: {dep} ({description})"
                )
            else:
        ...
        for dep, description in required_dev_deps.items():
            if dep not in dev_strings.lower():
                self.errors.append(
                    f"Missing required dev dependency: {dep} ({description})"
                )
            else:
            ...
            if not test_path.exists():
                ...
                with open(test_path) as f:
                    ...
                    if "def test_" not in content and "class Test" not in content:
                        self.warnings.append(
                            f"{test_file}: No test functions/classes found"
                        )
                    else:
        ...
        if gitleaks_path.exists():
            ...
            for pattern in airsernal_patterns:
                if pattern not in content:
                    self.warnings.append(
                        f"GitLeaks missing AIrsenal pattern: {pattern}"
                    )
                else:
    ...
    def validate_enhanced_components(self):
        ...
        for component_file, description in required_components.items():
            ...
            if not component_path.exists():
                self.warnings.append(
                    f"Missing component: {component_file} ({description})"
                )
            else:
                ...
                if "class " not in content and "def " not in content:
                    self.warnings.append(
                        f"{component_file}: No classes or functions found"
                    )
                else:
...
def main():
    ...
    if not (repo_root / "pyproject.toml").exists():
        ...
        print(
            "   Please run this script from the repository root or ensure pyproject.toml exists"
        )
        sys.exit(1)
