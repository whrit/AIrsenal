...
def test_weighted_performance_calculator():
    ...
    weighted_score = calculator.calculate_weighted_score(
...
def test_batch_vs_individual_consistency():
    ...
    for score, position in scores_and_positions:
        result = calculator.calculate_weighted_score(
            score, position, apply_difficulty_adjustment=False
        )
    ...
    for i, (batch_result, individual_result) in enumerate(
        ...
        assert diff < 1e-10, (
            f"Inconsistency at index {i}: batch={batch_result}, individual={individual_result}, diff={diff}"
        )
...
def test_performance_benchmarks():
    ...
    assert batch_time_ms < 50.0, (
        f"Batch calculation took {batch_time_ms:.2f}ms, exceeds 50ms target"
    )
    ...
    assert avg_individual_time_ms < 1.0, (
        f"Individual calculation averaging {avg_individual_time_ms:.4f}ms is too slow"
    )

    print(
        f"✓ Performance benchmarks passed (batch: {batch_time_ms:.2f}ms, individual avg: {avg_individual_time_ms:.4f}ms)"
    )
