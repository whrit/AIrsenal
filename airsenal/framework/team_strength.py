...
class TeamStrengthAnalyzer:
    ...
    def hierarchical_strength_model(
        ...
        mu_att = numpyro.sample("mu_att", dist.Normal(0.0, self.league_strength_prior_std))
        mu_def = numpyro.sample("mu_def", dist.Normal(0.0, self.league_strength_prior_std))
        
        sigma_att = numpyro.sample("sigma_att", dist.HalfNormal(self.team_strength_prior_std))
        sigma_def = numpyro.sample("sigma_def", dist.HalfNormal(self.team_strength_prior_std))
        
        # Home advantage
        home_advantage = numpyro.sample(
            "home_advantage", 
            dist.Normal(self.home_advantage_prior["mean"], self.home_advantage_prior["std"])
        )
        ...
        with numpyro.plate("matches", len(home_teams)):
            ...
            lambda_away = jnp.exp(
                attack[away_teams] - defense[home_teams]
            )
    ...
    def prepare_match_data(
        self, 
        season: str, 
        up_to_gameweek: Optional[int] = None
        ...
        """
        logger.info(f"Preparing match data for {season}, up to GW {up_to_gameweek}")
    ...
    def calculate_attacking_strength(
        ...
        """
        logger.debug(f"Calculating attacking strength for {team} ({'home' if home else 'away'})")
        ...
        weights = [self.alpha * (1 - self.alpha) ** i for i in range(len(recent_matches))]
        ...
        for i, (fixture, result) in enumerate(recent_matches):
            ...
            # Goals scored
            if home:
                match_goals = result.home_score
            else:
                match_goals = result.away_score
    ...
    def calculate_defensive_strength(
        ...
        """
        logger.debug(f"Calculating defensive strength for {team} ({'home' if home else 'away'})")
        ...
        for i, (fixture, result) in enumerate(recent_matches):
            ...
            # Goals conceded
            if home:
                match_goals_against = result.away_score
            else:
                match_goals_against = result.home_score
    ...
    def calculate_team_strengths(
        ...
        """
        logger.info(f"Calculating team strengths for {season} GW{gameweek}")
        ...
        try:
            ...
            for team_name in teams_to_analyze:
                if team_name not in team_mapping["team_to_idx"]:
                    logger.warning(f"Team {team_name} not found in match data")
                    continue
            ...
            logger.error(f"Error calculating team strengths: {e}")
            raise
    ...
    def _analyze_team_strength(
        ...
        home_attacking = self.calculate_attacking_strength(team_name, season, gameweek, True)
        away_attacking = self.calculate_attacking_strength(team_name, season, gameweek, False)
        home_defensive = self.calculate_defensive_strength(team_name, season, gameweek, True)
        away_defensive = self.calculate_defensive_strength(team_name, season, gameweek, False)
        ...
        return {
            ...
            "actual_goals_for_per_game": (
                home_attacking["goals_per_game"] + 
                away_attacking["goals_per_game"]
            ) / 2,
            ...
            "shots_for_per_game": (
                home_attacking["shots_per_game"] + 
                away_attacking["shots_per_game"]
            ) / 2,
            ...
            **external_factors,
            ...
            "convergence_diagnostic": self._calculate_rhat(attack_samples, defense_samples),
    ...
    def save_team_strength(self, strength_data: Dict) -> TeamStrength:
        ...
        history_data["matches_used_in_calculation"] = strength_data.get("matches_played", 0)
    ...
    def update_all_team_strengths(
        ...
        """
        logger.info(f"Updating all team strengths for {season} GW{gameweek}")
        ...
        for team_name, strength_data in all_strengths.items():
            try:
                ...
                logger.debug(f"Updated strength for {team_name}")
            except Exception as e:
                logger.error(f"Error saving strength for {team_name}: {e}")
        
        logger.info(f"Updated strengths for {len(updated_teams)} teams")
    ...
    def validate_strength_predictions(
        ...
        """
        logger.info(f"Validating strength predictions for {season}")
        ...
        correct_predictions = sum(
            1 for d in validation_data
            if (d["predicted_home_win"] and d["actual_home_win"]) or
               (not d["predicted_home_win"] and not d["actual_home_win"])
        )
    ...
    def _get_team_match_stats(
        ...
        expected_goals = sum(
            p.expected_goals or 0.0 for p in team_players
        )
    ...
    def _calculate_form_metrics(
        ...
        if len(goal_differences) > 1:
            ...
            trend_slope = stats.linregress(x, goal_differences[::-1])[0]  # Reverse for chronological order
...
def create_team_strength_analyzer(
    dbsession: Session,
    **kwargs
...
def get_all_team_strengths_for_api(
    ...
    for (team_name,) in teams:
        try:
            ...
            logger.error(f"Error getting strength for {team_name}: {e}")
            continue
...
def validate_team_strength_model(
    ...
    try:
        validation_results = analyzer.validate_strength_predictions(season)
        return validation_results
    except Exception as e:
        logger.error(f"Error validating team strength model: {e}")
