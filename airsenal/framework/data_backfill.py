...
class BackfillResult:
    ...
    def add_validation_issue(self, severity: ValidationSeverity, category: str, 
                           description: str, affected_entity: str, 
                           entity_id: Optional[int] = None,
                           suggested_action: Optional[str] = None,
                           **metadata):
    ...
    def has_critical_issues(self) -> bool:
        ...
        return any(issue.severity == ValidationSeverity.CRITICAL 
...
class BackfillProgressTracker:
    ...
    def __init__(self, session_id: str, total_items: int):
        ...
        self._callbacks = []
        self._persistence_path = Path(AIRSENAL_HOME) / "backfill_progress" / f"{session_id}.json"
    ...
    def update(self, completed_delta: int = 1, failed_delta: int = 0, 
               skipped_delta: int = 0, current_operation: str = ""):
        ...
        with self.lock:
            ...
            self.progress.last_update_time = datetime.now()
            self.progress.estimated_completion = self.progress.estimate_completion_time()
    ...
    def _save_progress(self):
        ...
        try:
            progress_data = {
                ...
                "last_update_time": self.progress.last_update_time.isoformat() if self.progress.last_update_time else None,
    ...
    def _load_progress(self):
        ...
        try:
            if self._persistence_path.exists():
                ...
                self.progress.current_operation = progress_data.get("current_operation", "")
                ...
                if progress_data.get("last_update_time"):
                    self.progress.last_update_time = datetime.fromisoformat(progress_data["last_update_time"])
...
class BackfillValidator:
    ...
    def validate_backfilled_data(self, season: str, gameweeks: Optional[List[int]] = None) -> List[ValidationIssue]:
        """Run comprehensive validation on backfilled data"""
        issues = []
        
        logger.info(f"Starting validation for {season}" + 
        ...
        logger.info(f"Validation completed with {len(issues)} total issues")
        return issues
    
    def _validate_xg_data_consistency(self, season: str, gameweeks: Optional[List[int]]) -> List[ValidationIssue]:
        ...
        with session_scope() as db_session:
            ...
            scores_with_xg = query.filter(
                PlayerScore.expected_goals.isnot(None)
            ).all()
            
            for score in scores_with_xg:
                # Validate xG + xA = xGI consistency
                if (score.expected_goals is not None and 
                    score.expected_assists is not None and
                    score.expected_goal_involvements is not None):
    ...
    def _validate_missing_data(self, season: str, gameweeks: Optional[List[int]]) -> List[ValidationIssue]:
        ...
        with session_scope() as db_session:
            ...
            coverage_percentage = (scores_with_xg / total_scores * 100) if total_scores > 0 else 0
            ...
            if gameweeks is None:
                gameweeks = [gw for gw, in db_session.query(Fixture.gameweek).filter(
    ...
    def _validate_statistical_outliers(self, season: str, gameweeks: Optional[List[int]]) -> List[ValidationIssue]:
        ...
        with session_scope() as db_session:
            ...
            if xg_stats and xg_stats.count_xg > 10:
                ...
                extreme_xg_scores = query.filter(
                    PlayerScore.expected_goals > 3.0
                ).all()
    ...
    def _validate_cross_references(self, season: str, gameweeks: Optional[List[int]]) -> List[ValidationIssue]:
        ...
        with session_scope() as db_session:
            ...
            orphaned_scores = db_session.query(PlayerScore).filter(
                and_(
                    PlayerScore.fixture.has(Fixture.season == season),
                    ~PlayerScore.player_id.in_(
                        db_session.query(Player.player_id)
                    )
                )
            )
    ...
    def _validate_temporal_consistency(self, season: str, gameweeks: Optional[List[int]]) -> List[ValidationIssue]:
...
class HistoricalDataBackfiller:
    ...
    def __init__(self, xg_manager: Optional[XGDataManager] = None):
    ...
    def configure(self, batch_size: int = 50, max_workers: int = 4, 
        ...
        self.logger.info(f"Configured backfiller: batch_size={batch_size}, "
                        f"max_workers={max_workers}, retry_attempts={retry_attempts}")
    
    def backfill_season(self, season: str, resume_session_id: Optional[str] = None) -> BackfillResult:
        """Backfill all data for a complete season"""
        session_id = resume_session_id or f"season_{season}_{int(time.time())}"
        
        self.logger.info(f"Starting season backfill for {season} (session: {session_id})")
        
        # Get all gameweeks for the season
        with session_scope() as db_session:
            gameweeks = [gw for gw, in db_session.query(Fixture.gameweek).filter(
                Fixture.season == season
            ).distinct().order_by(Fixture.gameweek).all()]
        ...
        self.logger.info(f"Found {len(gameweeks)} gameweeks for {season}: {gameweeks}")
        ...
        def log_progress(progress: BackfillProgress):
            if progress.completed_items % 5 == 0:  # Log every 5 gameweeks
                self.logger.info(
                    f"Season {season} backfill progress: {progress.progress_percentage:.1f}% "
        ...
        try:
            ...
            for i in range(0, len(gameweeks), self._batch_size):
                ...
                tracker.update(current_operation=f"Processing gameweeks {batch[0]}-{batch[-1]}")
            ...
            if failed_gameweeks:
                self.logger.warning(f"Retrying {len(failed_gameweeks)} failed gameweeks")
            ...
            if not failed_gameweeks and not result.has_critical_issues:
                ...
                self.logger.info(f"Season {season} backfill completed successfully")
            else:
                result.status = BackfillStatus.FAILED
                self.logger.error(f"Season {season} backfill failed. "
                                f"Failed gameweeks: {failed_gameweeks}")
            
            result.progress = tracker.progress
            
        except Exception as e:
            self.logger.error(f"Season {season} backfill failed with exception: {e}")
            result.status = BackfillStatus.FAILED
            result.add_validation_issue(
                ValidationSeverity.CRITICAL,
                "system_error",
    ...
    def backfill_gameweeks(self, season: str, gameweeks: List[int], 
                          session_id: Optional[str] = None) -> BackfillResult:
        """Backfill specific gameweeks"""
        session_id = session_id or f"gameweeks_{season}_{'_'.join(map(str, gameweeks))}_{int(time.time())}"
        
        self.logger.info(f"Starting gameweeks backfill for {season} GW{gameweeks} (session: {session_id})")
        ...
        try:
            ...
            # Validation
            validation_issues = self.validator.validate_backfilled_data(season, gameweeks)
            result.validation_issues.extend(validation_issues)
            
            # Reconciliation report
            result.reconciliation_report = self._generate_reconciliation_report(season, gameweeks)
            ...
            self.logger.error(f"Gameweeks backfill failed: {e}")
            result.status = BackfillStatus.FAILED
            result.add_validation_issue(
                ValidationSeverity.CRITICAL,
                "system_error",
    ...
    def _process_gameweek_batch(self, season: str, gameweeks: List[int]) -> Dict[int, BackfillResult]:
        ...
        with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
            ...
            for future in as_completed(future_to_gw):
                ...
                try:
                    ...
                    self.logger.error(f"Failed to process gameweek {gw}: {e}")
                    result = BackfillResult(
                        status=BackfillStatus.FAILED,
                        progress=BackfillProgress()
                    )
    ...
    def _backfill_single_gameweek(self, season: str, gameweek: int) -> BackfillResult:
        ...
        result = BackfillResult(
            status=BackfillStatus.RUNNING,
            progress=BackfillProgress(total_items=1)
        )
        
        try:
            self.logger.debug(f"Processing {season} GW{gameweek}")
            
            # Use existing xG manager to sync data
            updated_count = self.xg_manager.sync_xg_data_for_gameweek(season, gameweek)
            
            if updated_count > 0:
                self.logger.debug(f"Updated {updated_count} records for {season} GW{gameweek}")
                result.status = BackfillStatus.COMPLETED
                result.progress.completed_items = 1
            else:
                self.logger.warning(f"No data updated for {season} GW{gameweek}")
            ...
            self.logger.error(f"Failed to backfill {season} GW{gameweek}: {e}")
            result.status = BackfillStatus.FAILED
    ...
    def _retry_failed_gameweeks(self, season: str, gameweeks: List[int]) -> Dict[int, BackfillResult]:
        ...
        for gw in gameweeks:
            for attempt in range(self._retry_attempts):
                try:
                    self.logger.info(f"Retry attempt {attempt + 1}/{self._retry_attempts} for {season} GW{gw}")
                    ...
                    self.logger.warning(f"Retry {attempt + 1} failed for {season} GW{gw}: {e}")
    ...
    def _generate_reconciliation_report(self, season: str, 
                                      gameweeks: Optional[List[int]] = None) -> Dict[str, Any]:
        ...
        with session_scope() as db_session:
            ...
            total_scores = base_query.count()
            scores_with_xg = base_query.filter(PlayerScore.expected_goals.isnot(None)).count()
            scores_with_xa = base_query.filter(PlayerScore.expected_assists.isnot(None)).count()
            scores_with_xgi = base_query.filter(PlayerScore.expected_goal_involvements.isnot(None)).count()
            ...
            report["data_coverage"] = {
                ...
                "xg_coverage_percentage": (scores_with_xg / total_scores * 100) if total_scores > 0 else 0,
                "xa_coverage_percentage": (scores_with_xa / total_scores * 100) if total_scores > 0 else 0,
                "xgi_coverage_percentage": (scores_with_xgi / total_scores * 100) if total_scores > 0 else 0,
            }
            ...
            if scores_with_xg > 0:
                ...
                report["quality_metrics"] = {
                    "average_xg_per_player_per_match": float(xg_stats.avg_xg) if xg_stats.avg_xg else 0,
                    "maximum_xg_in_match": float(xg_stats.max_xg) if xg_stats.max_xg else 0,
                    "minimum_xg_in_match": float(xg_stats.min_xg) if xg_stats.min_xg else 0,
                }
            
            # Identify gaps by gameweek
            gw_query = gameweeks or [gw for gw, in db_session.query(Fixture.gameweek).filter(
                Fixture.season == season
            ).distinct().all()]
            ...
            if len(report["gaps_identified"]) > 0:
                high_priority_gaps = [gap for gap in report["gaps_identified"] if gap["severity"] == "high"]
                if high_priority_gaps:
                    report["recommendations"].append({
                        "priority": "high",
                        "action": "targeted_backfill",
                        "description": f"Prioritize backfill for {len(high_priority_gaps)} gameweeks with <50% coverage",
                        "gameweeks": [gap["gameweek"] for gap in high_priority_gaps]
                    })
    ...
    def incremental_backfill(self, season: str, days_back: int = 7) -> BackfillResult:
        ...
        self.logger.info(f"Starting incremental backfill for {season}, last {days_back} days")
        ...
        with session_scope() as db_session:
            ...
            self.logger.info(f"Found {len(target_gameweeks)} recent gameweeks: {sorted(target_gameweeks)}")
    ...
    def get_missing_data_report(self, season: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"Generating missing data report for {season}")
        ...
        return {
            ...
            "summary": {
                ...
                "critical_issues": len([i for i in missing_data_issues if i.severity == ValidationSeverity.CRITICAL]),
                "high_priority_gaps": len(reconciliation_report.get("gaps_identified", [])),
                "overall_coverage": reconciliation_report["data_coverage"]["xg_coverage_percentage"]
            }
        }