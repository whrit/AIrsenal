...
class PerformanceWeightMatrix:
    ...
    goalkeeper_weights: dict[str, float] = field(
    ...
    defender_weights: dict[str, float] = field(
        default_factory=lambda: {
    ...
    midfielder_weights: dict[str, float] = field(
        default_factory=lambda: {
    ...
    forward_weights: dict[str, float] = field(
        default_factory=lambda: {
    ...
    difficulty_adjustments: dict[int, float] = field(
        default_factory=lambda: {
    ...
    def get_weights_for_position(self, position: str) -> dict[str, float]:
        ...
        if position not in position_weights:
            msg = f"Unknown position: {position}. Must be one of {list(position_weights.keys())}"
...
class WeightedPerformanceCalculator:
    ...
    def __init__(
        self,
        weight_matrix: PerformanceWeightMatrix | None = None,
    ...
    def extract_metrics_from_score(
        self, score: PlayerScore, position: str
    ) -> dict[str, float]:
        """
            ...
            metrics.update(
                {
                    ...
                    "penalty_taker_boost": self._get_penalty_taker_boost(
                        score.player_id
                    ),
                }
            )
    ...
    def _calculate_per_90_metric(
        self, metric_base: str, score: PlayerScore, minutes: int
    ) -> float:
    ...
    def _get_penalty_taker_boost(self, player_id: int | None) -> float:
    ...
    def calculate_weighted_score(
        ...
        score: PlayerScore,
        position: str,
        apply_difficulty_adjustment: bool = True,
    ) -> float:
        """
        ...
        for metric_name in sorted(weights.keys()):
            if metric_name in weights:
                ...
                normalized_value = self.weights.normalize_metric(
    ...
    def calculate_batch_scores(
        ...
        position_groups: dict[str, list[tuple[int, PlayerScore]]] = {}
        ...
        for position, score_indices in position_groups.items():
            ...
            weights_vector = self._build_weights_vector(
                position_weights, metrics_matrix.shape[1]
            )
    ...
    def _build_metrics_matrix(
        self, scores: list[PlayerScore], position: str
    ) -> np.ndarray:
    ...
    def _build_weights_vector(
        self, position_weights: dict[str, float], num_metrics: int
    ) -> np.ndarray:
    ...
    def calculate_historical_performance(
        self, player_id: int, season: str, num_gameweeks: int = 5
    ) -> dict[str, float]:
        """
        ...
        # Get player position
        player = (
            self.dbsession.query(Player).filter(Player.player_id == player_id).first()
        )
        if not player:
            return {"count": 0, "mean_score": 0.0, "trend": 0.0}
...
def calculate_weighted_performance(
    score: PlayerScore,
    position: str,
    calculator: WeightedPerformanceCalculator | None = None,
) -> float:
    """
...
def calculate_batch_weighted_performance(
    scores_and_positions: list[tuple[PlayerScore, str]],
    calculator: WeightedPerformanceCalculator | None = None,
) -> np.ndarray:
    """
