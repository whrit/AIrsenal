...
class Alert:
    ...
    change_points: list[ChangePoint] | None
    timestamp: datetime
...
class TrendDetector:
    ...
    def __init__(
        ...
        if not SCIPY_AVAILABLE:
            logger.warning(
                "scipy not available. Statistical tests will use simplified implementations."
            )
    ...
    def _get_player_performance_data(
        ...
        if len(data) < self.min_data_points:
            msg = f"Insufficient data for player {player_id}: {len(data)} points < {self.min_data_points} required"
    ...
    def _get_fixture_difficulty(
        self, opponent: str, player_id: int, gameweek: int
    ) -> float:
        ...
        try:
            ...
            difficulty_info = get_fixture_difficulty(
                fixture.fixture_id, perspective_team=team, dbsession=self.dbsession
            )
    ...
    def _adjust_points_for_difficulty(
        self, points: pd.Series, difficulty: pd.Series
    ) -> pd.Series:
        """
        ...
        # Apply adjustment
        return points / adjustment_factor
    ...
    def mann_kendall_test(self, data: np.ndarray) -> tuple[float, float, float]:
        ...
        if SCIPY_AVAILABLE:
            ...
            try:
                ...
                # Handle NaN case for zero variance data
                if np.isnan(tau):
                    tau = 0.0
                ...
                logger.warning(
                    f"scipy Mann-Kendall failed, using manual implementation: {e}"
                )
    ...
    def detect_trends(
        ...
        try:
            ...
            p_value_str = f"{p_value:.3f}" if p_value is not None else "N/A"
            logger.debug(
                f"Trend analysis for player {player_id}: {direction.value} "
                f"(slope={slope:.3f}, p={p_value_str})"
            )
            ...
            msg = f"Trend detection failed: {e}"
            raise TrendDetectionError(msg)
        ...
        for i in range(0, total_players, chunk_size):
            ...
            for player_id in chunk_ids:
                try:
                    ...
                    logger.warning(
                        f"Failed to detect trends for player {player_id}: {e}"
                    )
                    continue
                except Exception as e:
...
class ChangePointDetector:
    ...
    def _cost_function(self, data: np.ndarray, model: str = "normal") -> float:
        ...
        if model == "normal":
            ...
            return len(data) * (0.5 * np.log(2 * np.pi * var) + 0.5)
        ...
        if model == "poisson":
            ...
            return len(data) * mean - np.sum(data) * np.log(mean)
        ...
        return len(data) * np.log(mean) + np.sum(data) / mean
    ...
    def _binary_segmentation(
        self, data: np.ndarray, max_change_points: int = 5
    ) -> list[int]:
        """
        ...
        def find_best_change_point(segment_data, start_idx):
            ...
            for i in range(
                self.min_segment_length, len(segment_data) - self.min_segment_length
            ):
    ...
    def find_breakpoints(
        ...
        """
        # Validate algorithm before trying anything else
        if algorithm not in ["pelt", "binary_segmentation"]:
        ...
        try:
            ...
            for pos in change_point_positions:
                if 0 <= pos < len(df):
                    ...
                    if pos > 0 and pos < len(data) - 1:
                        ...
                        separate_cost = self._cost_function(
                            before_data, self.model
            ...
            msg = f"Change point detection failed: {e}"
            raise TrendDetectionError(msg)
    ...
    def batch_find_breakpoints(
        ...
        for player_id in player_ids:
            try:
                ...
                logger.warning(
                    f"Failed to find breakpoints for player {player_id}: {e}"
                )
                results[player_id] = []
            except Exception as e:
                logger.error(f"Unexpected error for player {player_id}: {e}")
...
class TrendVisualizer:
    ...
    def plot_trend_analysis(
        ...
        fig.suptitle(
            f"Player {player_id} Trend Analysis", fontsize=16, fontweight="bold"
        )
        ...
        if data is not None:
            ...
            trend_line = (
                x * trend_result.slope
                + np.mean(points)
                - np.mean(x) * trend_result.slope
            )
            ax1.plot(
                gameweeks,
                trend_line,
            ...
            if len(points) >= 26:  # Need enough data for MACD
                ...
                if len(macd) > 0:
                    ...
                    ax2.bar(
                        macd_x, histogram, alpha=0.3, color="gray", label="Histogram"
                    )
    ...
    def plot_batch_trends(
        ...
        ax1.pie(
            direction_counts.values, labels=direction_counts.index, autopct="%1.1f%%"
        )
        ...
        ax2.set_xticklabels(
            [s.replace("_", " ").title() for s in significance_counts.index],
            rotation=45,
        )
...
class AlertSystem:
    ...
    def check_significant_changes(
        ...
        for player_id in player_ids:
            try:
                # Get player name
                player = (
                    self.dbsession.query(Player).filter_by(player_id=player_id).first()
                )
                ...
                if player_id in trend_results:
                    ...
                    trend_alerts = self._check_trend_alerts(
                        player_id, player_name, trend_result
                    )
        ...
        alerts.sort(
            key=lambda x: (severity_order.get(x.severity, 4), x.timestamp), reverse=True
        )
    ...
    def _check_trend_alerts(
        self, player_id: int, player_name: str, trend_result: TrendResult
    ) -> list[Alert]:
        ...
        if (
            trend_result.direction == TrendDirection.IMPROVING
            and trend_result.p_value < self.significance_threshold
            and trend_result.slope > self.slope_threshold
        ):
        ...
        # Significant declining trend alert
            ...
            trend_result.direction == TrendDirection.DECLINING
            and trend_result.p_value < self.significance_threshold
            and abs(trend_result.slope) > self.slope_threshold
        ):
        ...
        # Highly significant but small slope (stable but significant)
            ...
            trend_result.significance == TrendSignificance.HIGHLY_SIGNIFICANT
            and abs(trend_result.slope) < 0.1
        ):
            message = (
    ...
    def _check_change_point_alerts(
        self, player_id: int, player_name: str, change_points: list[ChangePoint]
    ) -> list[Alert]:
        ...
        if high_confidence_cps:
            ...
            recent_cps = [
                cp for cp in high_confidence_cps if NEXT_GAMEWEEK - cp.gameweek <= 5
            ]
    ...
    def generate_alert_report(self, alerts: list[Alert], format: str = "text") -> str:
        """
    ...
    def _generate_html_report(self, alerts: list[Alert]) -> str:
        ...
        html_lines = [
            ...
            "</head>",
            "<body>",
            "<h1>AIrsenal Trend Alert Report</h1>",
