...
class ABTestManager:
    ...
    def create_experiment(
        ...
        try:
            control_model = self.model_manager.load_model_version(
                config.model_name, config.control_version
            )
            treatment_model = self.model_manager.load_model_version(
                config.model_name, config.treatment_version
            )
        except Exception as e:
            raise ValueError(f"Cannot load model versions: {e}")
        
        # Get model version IDs
        registry = self.dbsession.query(
            self.model_manager.dbsession.query(
                self.model_manager.model_manager.ModelRegistry
        ...
        control_version_record = self.dbsession.query(
            self.model_manager.ModelVersion
        ).filter_by(
            registry_id=registry.id, version=config.control_version
        ).first()
        
        treatment_version_record = self.dbsession.query(
            self.model_manager.ModelVersion
        ).filter_by(
            registry_id=registry.id, version=config.treatment_version
        ).first()
        
        if not control_version_record or not treatment_version_record:
            raise ValueError("Model version records not found")
        
        # Create experiment record
        experiment = ModelExperiment(
            experiment_name=config.experiment_name,
            description=description or f"A/B test: {config.control_version} vs {config.treatment_version}",
        ...
        logger.info(f"Created experiment '{config.experiment_name}' (ID: {experiment.id})")
        return experiment
    
    def start_experiment(self, experiment_id: int) -> ModelExperiment:
        """Start a planned experiment"""
        
        experiment = self.dbsession.query(ModelExperiment).filter_by(id=experiment_id).first()
        ...
        logger.info(f"Started experiment '{experiment.experiment_name}' (ID: {experiment.id})")
        return experiment
    ...
    def stop_experiment(
        ...
        if experiment.status != ExperimentStatus.RUNNING.value:
            raise ValueError(f"Cannot stop experiment in status '{experiment.status}'")
    ...
    def run_experiment_iteration(
        ...
        sample_players: Optional[List[int]] = None,
    ) -> Dict[str, str]:
        """
        ...
        if experiment.status != ExperimentStatus.RUNNING.value:
            raise ValueError(f"Experiment must be running to iterate (current: {experiment.status})")
        
        # Get model versions
        control_version = self.dbsession.query(self.model_manager.ModelVersion).filter_by(
            id=experiment.control_version_id
        ).first()
        treatment_version = self.dbsession.query(self.model_manager.ModelVersion).filter_by(
            id=experiment.model_version_id
        ).first()
        ...
        model_name = control_version.registry.model_name
    ...
    def analyze_experiment_results(
        ...
        if control_predictions.empty or treatment_predictions.empty:
            raise ValueError("No predictions found for experiment groups")
        ...
        analysis = {
            ...
            "treatment_mean_prediction": treatment_predictions["predicted_points"].mean(),
        ...
        if actual_results is not None:
            ...
            analysis["accuracy_improvement"] = (
                treatment_accuracy.get("mae", 0) - control_accuracy.get("mae", 0)
            )
    ...
    def check_early_stopping(
        ...
        if min_sample < config.min_sample_size:
            return False, f"Sample size too small: {min_sample} < {config.min_sample_size}"
    ...
    def get_experiment_history(
        ...
        if model_name:
            ...
            query = query.join(
                self.model_manager.ModelVersion,
                ModelExperiment.model_version_id == self.model_manager.ModelVersion.id
            ).join(
                self.model_manager.ModelRegistry,
                self.model_manager.ModelVersion.registry_id == self.model_manager.ModelRegistry.id
            ).filter(
                self.model_manager.ModelRegistry.model_name == model_name
            )
    ...
    def _calculate_prediction_accuracy(
        ...
        if "predicted_points" in predictions.columns and "actual_points" in actual_results.columns:
    ...
    def _generate_recommendation(self, analysis: Dict[str, Any]) -> str:
        ...
        treatment_better = analysis.get("treatment_mean_prediction", 0) > analysis.get("control_mean_prediction", 0)
        
        if is_significant and effect_size >= 0.2:
            if treatment_better:
                return "Deploy treatment version - statistically significant improvement"
            else:
