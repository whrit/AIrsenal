...
from airsenal.framework.env import save_env, SPORTMONKS_API_KEY
from airsenal.framework.logging_config import get_logger
from airsenal.framework.logging_utils import api_logger, log_api_call, timed
from airsenal.framework.schema import PlayerScore, Player, Fixture, session_scope
...
class XGDataPoint:
    ...
    def __post_init__(self):
        ...
        if (self.expected_goals is not None and 
            self.expected_assists is not None and 
            self.expected_goal_involvements is None):
            self.expected_goal_involvements = self.expected_goals + self.expected_assists
...
class XGDataProvider(ABC):
    ...
    def __init__(self, config: ProviderConfig):
        ...
        self._setup_session()
        logger.info(f"Initialized {self.__class__.__name__} with tier {config.tier.value}")
    ...
    @abstractmethod
    def get_match_xg_data(self, fixture_id: int) -> List[XGDataPoint]:
        """Fetch xG/xA data for all players in a specific match"""
        pass
    
    @abstractmethod
    def get_bulk_xg_data(
        self, 
    ...
    def _make_request(
        ...
        if not self.config.api_key and not self._is_credentials_optional():
            logger.warning("No API key configured - some providers may return limited data")
...
class DataTransformer:
        ...
        try:
            ...
            for stat in statistics:
                ...
                xg_point = XGDataPoint(
                    ...
                    expected_goal_involvements=xg_stats.get("expected_goal_involvements"),
                    expected_goals_conceded=xg_stats.get("expected_goals_conceded"),
                    raw_data=stat,
                    provider="Sportmonks",
                    data_source_tier=self.config.tier if hasattr(self, 'config') else ProviderTier.BASIC
                )
    ...
    def sportmonks_bulk_to_xg_data(
        ...
        try:
            ...
            for fixture in fixtures:
                ...
                for stat in statistics:
                    ...
                    xg_point = XGDataPoint(
                        player_id=player_id,
                        fixture_id=fixture_id,
                        match_date=self._parse_sportmonks_date(fixture.get("starting_at")),
                        expected_goals=xg_stats.get("expected_goals"),
                        expected_assists=xg_stats.get("expected_assists"),
                        expected_goal_involvements=xg_stats.get("expected_goal_involvements"),
                        expected_goals_conceded=xg_stats.get("expected_goals_conceded"),
            ...
            logger.error(
                f"Error transforming Sportmonks bulk data",
                extra={"error": str(e), "response_data": response_data}
            )
        
        return xg_data
    
    def _extract_xg_stats_from_sportmonks(self, stat: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _parse_sportmonks_date(self, date_str: Optional[str]) -> datetime:
...
class APIHealthMonitor:
    ...
    def _is_provider_healthy(self, provider: XGDataProvider) -> bool:
        ...
        if metrics.requests_made > 0 and metrics.success_rate < 0.8:
            return False
    ...
    def get_cost_summary(self) -> Dict[str, Any]:
        ...
        for name, provider in self.providers.items():
            ...
            provider_costs[name] = {
                ...
                "estimated_monthly_cost": metrics.total_cost * 30 if metrics.total_cost > 0 else 0
            }
...
class XGDataManager:
    ...
    def setup_sportmonks(self, api_key: Optional[str] = None) -> bool:
    ...
    @timed
    def sync_xg_data_for_gameweek(self, season: str, gameweek: int) -> int:
    ...
    def _update_player_scores(self, xg_data: List[XGDataPoint], session) -> int:
        ...
        for xg_point in xg_data:
            try:
                ...
                player_score = session.query(PlayerScore).filter(
                ...
                if player_score:
                    ...
                    player_score.expected_goal_involvements = xg_point.expected_goal_involvements
                    player_score.expected_goals_conceded = xg_point.expected_goals_conceded
...
def create_xg_manager(sportmonks_api_key: Optional[str] = None) -> XGDataManager:
    """Create and configure xG data manager with Sportmonks provider"""
    manager = XGDataManager()
    
    if sportmonks_api_key or True:  # Allow setup even without key for graceful degradation
