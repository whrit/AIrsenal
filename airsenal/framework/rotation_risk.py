...
from airsenal.framework.utils import (
    CURRENT_SEASON, get_fixtures_for_player, get_player, get_recent_scores_for_player
)
...
class ManagerPatternAnalyzer:
    ...
    def analyze_manager_patterns(self, team: str, season: str, manager_name: str = None) -> Dict[str, float]:
        """
        ...
        patterns = {
            ...
            'position_preferences': {
                'GK': 0.1, 'DEF': 0.3, 'MID': 0.4, 'FWD': 0.35
            },
        ...
        try:
            ...
            for i, fixture in enumerate(fixtures):
                ...
                lineup = {
                    ...
                    'date': fixture.date,
                    'starters': [s for s in scores if s.minutes >= 60],  # Played most of match
                    'rotated_in': [s for s in scores if 0 < s.minutes < 60],  # Substituted 
                ...
                if i > 0:
                    ...
                    rotation_events.extend(
                        self._detect_rotation_events(prev_lineup, lineup, fixtures[i-1], fixture)
                    )
            
            # Calculate rotation patterns from events
            if rotation_events:
                patterns = self._calculate_rotation_metrics(rotation_events, team_lineups)
    ...
    def _get_competition_type(self, fixture: Fixture) -> str:
        ...
        if hasattr(fixture, 'tag') and fixture.tag:
            ...
            elif 'EL' in fixture.tag or 'Europa' in fixture.tag:
                return 'Europa League'
            elif 'FA Cup' in fixture.tag:
                return 'FA Cup'
            elif 'Carabao' in fixture.tag or 'League Cup' in fixture.tag:
                return 'Carabao Cup'
        return 'Premier League'
    
    def _detect_rotation_events(self, prev_lineup: Dict, curr_lineup: Dict, 
                               prev_fixture: Fixture, curr_fixture: Fixture) -> List[Dict]:
    ...
    def _calculate_days_between_fixtures(self, prev_fixture: Fixture, curr_fixture: Fixture) -> int:
    ...
    def _calculate_rotation_metrics(self, rotation_events: List[Dict], 
                                  team_lineups: List[Dict]) -> Dict[str, Any]:
        """Calculate rotation pattern metrics from detected events."""
        total_fixtures = len(team_lineups)
        total_rotation_events = len([e for e in rotation_events if e['event_type'] == 'rotated_out'])
        
        # Overall rotation rate
        avg_rotation_rate = total_rotation_events / (total_fixtures * 11) if total_fixtures > 0 else 0.0
        
        # Congestion response (increased rotation when fixtures close together)
        congestion_events = [e for e in rotation_events if e['days_between_fixtures'] <= 3]
        normal_events = [e for e in rotation_events if e['days_between_fixtures'] > 3]
        
        congestion_rate = len(congestion_events) / max(1, len([e for e in rotation_events if e['days_between_fixtures'] <= 3]))
        normal_rate = len(normal_events) / max(1, len([e for e in rotation_events if e['days_between_fixtures'] > 3]))
        
        congestion_response = (congestion_rate / max(0.01, normal_rate)) if normal_rate > 0 else 1.0
        
        return {
            'avg_rotation_rate': min(1.0, avg_rotation_rate),
            'congestion_response': min(2.0, congestion_response),
            'position_preferences': self._calculate_position_rotation_rates(rotation_events),
            'competition_priorities': self._calculate_competition_priorities(rotation_events),
            'sample_size': total_fixtures
        }
    
    def _calculate_position_rotation_rates(self, rotation_events: List[Dict]) -> Dict[str, float]:
    ...
    def _calculate_competition_priorities(self, rotation_events: List[Dict]) -> Dict[str, float]:
...
class FixtureCongestionAnalyzer:
    ...
    def calculate_congestion_score(self, team: str, target_gameweek: int, 
                                  season: str = CURRENT_SEASON) -> Dict[str, float]:
        """
        ...
        try:
            ...
            target_fixture = next((f for f in fixtures if f.gameweek == target_gameweek), None)
            ...
            if target_date:
                for fixture in fixtures:
                    ...
                    if fixture_date and fixture.fixture_id != target_fixture.fixture_id:
                        ...
                        if fixture.away_team == team:
                            congestion_metrics['travel_burden'] += 1.5 if days_diff <= 7 else 1.0
                        else:
                            congestion_metrics['travel_burden'] += 1.0 if days_diff <= 7 else 0.5
                ...
                if recent_fixtures:
                    ...
                    if last_date:
                        congestion_metrics['recovery_time'] = (target_date - last_date).days
            ...
            logger.error(f"Error calculating congestion for {team} GW{target_gameweek}: {e}")
            return self._get_default_congestion_score()
...
class RotationRiskCalculator:
    ...
    def calculate_rotation_risk(self, player_id: int, gameweek: int, 
                               season: str = CURRENT_SEASON) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not attrs:
                raise ValueError(f"No attributes found for player {player_id} in {season} GW{gameweek}")
                
            team = attrs.team if not isinstance(attrs, tuple) else attrs[0].team
            position = attrs.position if not isinstance(attrs, tuple) else attrs[0].position
            
            # Calculate individual risk factors
            risk_factors = self._calculate_risk_factors(player_id, gameweek, season, team, position)
            ...
            logger.error(f"Error calculating rotation risk for player {player_id} GW{gameweek}: {e}")
            return self._get_default_risk_prediction(player_id, gameweek, season)
    
    def calculate_batch_rotation_risks(self, player_ids: List[int], gameweek: int, 
                                     season: str = CURRENT_SEASON) -> Dict[int, Dict[str, Any]]:
        """
        ...
        for player_id in player_ids:
            try:
                results[player_id] = self.calculate_rotation_risk(player_id, gameweek, season)
            except Exception as e:
                logger.error(f"Error in batch calculation for player {player_id}: {e}")
                results[player_id] = self._get_default_risk_prediction(player_id, gameweek, season)
                
        return results
    
    def _calculate_risk_factors(self, player_id: int, gameweek: int, season: str, 
                               team: str, position: str) -> Dict[str, float]:
        """Calculate individual risk factors for rotation prediction."""
        factors = {}
        
        # 1. Fixture congestion analysis
        congestion = self.congestion_analyzer.calculate_congestion_score(team, gameweek, season)
        factors['fixture_congestion'] = congestion['congestion_score']
        factors['recovery_time'] = min(1.0, max(0.0, 1.0 - congestion['recovery_time'] / 7.0))
        
        # 2. Recent minutes analysis (fatigue factor)
        factors['fatigue_risk'] = self._calculate_fatigue_risk(player_id, gameweek, season)
        ...
        factors['position_rotation_rate'] = manager_patterns['position_preferences'].get(position, 0.3)
        
        # 5. Player importance factor
        factors['player_importance'] = self._calculate_player_importance(player_id, team, season)
        
        # 6. Competition importance
        factors['competition_importance'] = self._get_competition_importance_factor(gameweek, season)
        
        # 7. Team depth factor
        factors['team_depth'] = self._calculate_team_depth_factor(team, position, season)
        
        return factors
    
    def _calculate_fatigue_risk(self, player_id: int, gameweek: int, season: str) -> float:
        """Calculate fatigue risk based on recent minutes played."""
        try:
            player = get_player(player_id, dbsession=self.dbsession)
            recent_scores = get_recent_scores_for_player(player, n_gameweeks=3, dbsession=self.dbsession)
            ...
            recent_minutes = []
            for gw, score in recent_scores.items():
                if gw >= gameweek - 3:  # Last 3 gameweeks
                    score_obj = (self.dbsession.query(PlayerScore)
                               .filter(
                                   PlayerScore.player_id == player_id,
                                   PlayerScore.fixture_id.in_(
                                       self.dbsession.query(Fixture.fixture_id)
                                       .filter(Fixture.gameweek == gw, Fixture.season == season)
                                   )
                               )
                               .first())
                    if score_obj:
    ...
    def _calculate_player_importance(self, player_id: int, team: str, season: str) -> float:
        ...
        try:
            ...
            for gw in range(max(1, self.dbsession.query(Fixture.gameweek).filter(
                Fixture.season == season).order_by(Fixture.gameweek.desc()).first()[0] - 10),
                self.dbsession.query(Fixture.gameweek).filter(
                    Fixture.season == season).order_by(Fixture.gameweek.desc()).first()[0] + 1):
            ...
            avg_points = total_points / total_available_matches if total_available_matches > 0 else 0
    ...
    def _calculate_team_depth_factor(self, team: str, position: str, season: str) -> float:
        ...
        try:
            ...
            position_players = (self.dbsession.query(PlayerAttributes)
                               .filter(
                                   PlayerAttributes.team == team,
                                   PlayerAttributes.season == season,
            ...
            if position == 'GK':
                ...
                depth_factor = min(1.0, (position_players - 4) / 4.0)  # 4-8 DEFs typical
            elif position == 'MID':
                depth_factor = min(1.0, (position_players - 4) / 6.0)  # 4-10 MIDs typical
            else:  # FWD
                depth_factor = min(1.0, (position_players - 2) / 4.0)  # 2-6 FWDs typical
    ...
    def _predict_with_ml_model(self, factors: Dict[str, float]) -> float:
        ...
        try:
            ...
            ensemble_pred = 0.6 * forest_pred + 0.4 * logistic_pred
    ...
    def _get_default_risk_prediction(self, player_id: int, gameweek: int, season: str) -> Dict[str, Any]:
    ...
    def train_model(self, training_data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            y = training_data['was_rotated']  # Binary target: 1 if rotated, 0 if started
            ...
            self.feature_importance = dict(zip(feature_columns, self.forest_model.feature_importances_))
            ...
            metrics = {
                ...
                'ensemble_accuracy': (logistic_scores.mean() + forest_scores.mean()) / 2,
                'feature_importance': self.feature_importance,
                'training_samples': len(training_data)
            }
            
            logger.info(f"Model training completed. Accuracy: {metrics['forest_accuracy']:.3f}")
            return metrics
    ...
    def validate_historical_accuracy(self, validation_data: pd.DataFrame) -> Dict[str, float]:
        """
...
def get_rotation_risk_for_player(player_id: int, gameweek: int, 
                                season: str = CURRENT_SEASON) -> Dict[str, Any]:
    """
...
def get_team_rotation_risks(team: str, gameweek: int, 
                           season: str = CURRENT_SEASON) -> Dict[int, Dict[str, Any]]:
    """
