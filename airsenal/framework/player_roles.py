...
class PenaltyStatistics:
    ...
    def __init__(self, player_id: int, player_name: str, team: str):
        ...
        self.last_penalty_date: Optional[datetime] = None
...
class PenaltyTakerAnalyzer:
    ...
    def get_team_penalty_statistics(self, team: str, season: str, 
                                  min_penalties: int = 10) -> Dict[int, PenaltyStatistics]:
        """
        ...
        with session_scope() as session:
            ...
            query = session.query(PlayerScore, Player, Fixture).join(
                Player, PlayerScore.player_id == Player.player_id
            ).join(
                Fixture, PlayerScore.fixture_id == Fixture.fixture_id
            ).filter(
                and_(
    ...
    def _estimate_penalty_goal_likelihood(self, score: PlayerScore, 
                                        fixture: Fixture, session: Session) -> float:
        """
        ...
        attrs = session.query(PlayerAttributes).filter(
            and_(
    ...
    def calculate_penalty_taker_confidence(self, stats: PenaltyStatistics, 
                                         team_total_penalties: int) -> float:
        """
    ...
    def identify_team_penalty_takers(self, team: str, season: str) -> List[Dict]:
        ...
        penalty_takers.sort(key=lambda x: x['confidence'], reverse=True)
    ...
    def _fallback_to_attributes(self, team: str, season: str) -> List[Dict]:
        ...
        """
        with session_scope() as session:
            query = session.query(PlayerAttributes, Player).join(
    ...
    def get_all_current_penalty_takers(self, season: str) -> Dict[str, List[Dict]]:
        ...
        with session_scope() as session:
            ...
            teams_query = session.query(PlayerAttributes.team).filter(
                PlayerAttributes.season == season
            ).distinct()
            ...
            for team, penalty_takers in all_penalty_takers.items():
                ...
                for pt in penalty_takers:
                    session.query(PlayerAttributes).filter(
                        ...
                        'role_confidence': pt['confidence']
                    })
                    updates_made += 1
                    
            session.commit()
            
        logger.info(f"Updated {updates_made} penalty taker assignments for season {season}")
        return updates_made
    ...
    def validate_historical_accuracy(self, test_seasons: List[str]) -> float:
        ...
        logger.info(f"Historical accuracy: {accuracy:.2%} ({correct_predictions}/{total_predictions})")
        return accuracy
