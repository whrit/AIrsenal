...
class FormCalculator:
    ...
    def _register_form_features(self) -> None:
        ...
        for feature_config in form_features:
            try:
                self.feature_store.register_feature(**feature_config, replace_existing=True)
            except Exception as e:
                logger.warning(f"Failed to register form feature {feature_config['name']}: {e}")
    ...
    def _get_player_scores_data(
        ...
        if not data:
            raise InsufficientDataError(f"No valid scores found for player {player_id} in season {season}")
    ...
    def calculate_rolling_form(
        ...
        try:
            ...
            df = self._get_player_scores_data(player_id, season, current_gameweek, gameweeks * 2)
            
            # Check if we have sufficient data
            if len(df) < self.min_games_for_form:
                logger.debug(f"Insufficient data for player {player_id}: {len(df)} games < {self.min_games_for_form} required")
                return None
            ...
            logger.debug(f"Insufficient data for rolling form calculation: player {player_id}")
            return None
        except Exception as e:
            logger.error(f"Failed to calculate rolling form for player {player_id}: {e}")
            raise FormCalculationError(f"Rolling form calculation failed: {e}")
        finally:
    ...
    def calculate_weighted_form(
        ...
        try:
            ...
            logger.debug(f"Insufficient data for weighted form calculation: player {player_id}")
            return None
        except Exception as e:
            logger.error(f"Failed to calculate weighted form for player {player_id}: {e}")
            raise FormCalculationError(f"Weighted form calculation failed: {e}")
        finally:
            calculation_time = time.time() - start_time
            self._calculation_times.append(calculation_time)
    ...
    def calculate_momentum(
        ...
        try:
            ...
            # Retrieve player performance data
            df = self._get_player_scores_data(player_id, season, current_gameweek, historical_window * 2)
            
            # Check if we have sufficient data for both windows
            if len(df) < historical_window:
                logger.debug(f"Insufficient data for momentum calculation: player {player_id} has {len(df)} games < {historical_window} required")
                return None
            ...
            logger.debug(f"Insufficient data for momentum calculation: player {player_id}")
            return None
        except Exception as e:
            logger.error(f"Failed to calculate momentum for player {player_id}: {e}")
            raise FormCalculationError(f"Momentum calculation failed: {e}")
        finally:
            calculation_time = time.time() - start_time
            self._calculation_times.append(calculation_time)
    ...
    def batch_calculate_form(
        ...
        logger.info(f"Starting batch form calculation for {total_players} players")
        ...
        try:
            ...
            for i in range(0, total_players, chunk_size):
                ...
                logger.debug(f"Processing chunk {i//chunk_size + 1}/{(total_players + chunk_size - 1)//chunk_size}: "
                           f"players {i} to {min(i + chunk_size, total_players)}")
                
                # Calculate form metrics for each player in the chunk
                for player_id in chunk_player_ids:
                    player_results: Dict[str, Optional[float]] = {}
                    ...
                    try:
                        ...
                        logger.error(f"Failed to calculate form for player {player_id}: {e}")
                ...
                logger.debug(f"Chunk {i//chunk_size + 1} completed in {chunk_time:.3f}s")
            ...
            logger.info(f"Batch calculation completed: {successful_calculations}/{total_players} players successful "
                       f"in {total_time:.3f}s ({total_players/total_time:.1f} players/sec)")
            
            return results
            
        except Exception as e:
            logger.error(f"Batch form calculation failed: {e}")
            raise FormCalculationError(f"Batch calculation failed: {e}")
    ...
    def update_player_attributes(
        ...
        try:
            ...
            # Find or create PlayerAttributes record
            player_attr = (
            ...
            if not player_attr:
                logger.debug(f"No PlayerAttributes found for player {player_id} GW{gameweek} {season}")
                return False
            ...
            logger.debug(f"Updated form metrics for player {player_id} GW{gameweek}: "
                        f"form_3={form_3:.2f if form_3 else None}, "
                        f"form_5={form_5:.2f if form_5 else None}, "
                        f"momentum={momentum:.2f if momentum else None}")
            
            return True
            
        except Exception as e:
            logger.error(f"Failed to update player attributes for player {player_id}: {e}")
            if commit:
                self.dbsession.rollback()
            raise FormCalculationError(f"Failed to update player attributes: {e}")
    ...
    def batch_update_player_attributes(
        ...
        for i in range(0, len(player_ids), chunk_size):
            ...
            try:
                    ...
                    try:
                        ...
                        if player_attr:
                            ...
                            if player_results.get('form_3_games') is not None:
                                player_attr.form_3_games = player_results['form_3_games']
                            if player_results.get('form_5_games') is not None:
                                player_attr.form_5_games = player_results['form_5_games']
                            if player_results.get('form_10_games') is not None:
                                player_attr.form_10_games = player_results['form_10_games']
                            if player_results.get('momentum') is not None:
                                player_attr.momentum = player_results['momentum']
                            
                            successful_updates += 1
                        else:
                            logger.warning(f"No PlayerAttributes found for player {player_id} GW{gameweek} {season}")
                            failed_updates += 1
                ...
                # Commit the chunk
                self.dbsession.commit()
                logger.debug(f"Committed updates for chunk {i//chunk_size + 1}")
        ...
        logger.info(f"Batch update completed: {successful_updates}/{total_processed} successful")
        return stats
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if not self._calculation_times:
            return {
                ...
                "cache_performance": {"hits": self._cache_hits, "misses": self._cache_misses},
    ...
    def validate_form_calculations(
        ...
        try:
            ...
            validation_results["calculations"] = {
                "form_3_games": self.calculate_rolling_form(player_id, 3, season, gameweek, use_cache=False),
                "form_5_games": self.calculate_rolling_form(player_id, 5, season, gameweek, use_cache=False),
                "form_10_games": self.calculate_rolling_form(player_id, 10, season, gameweek, use_cache=False),
                "weighted_form": self.calculate_weighted_form(player_id, 10, season, gameweek, use_cache=False),
                "momentum": self.calculate_momentum(player_id, season, gameweek, use_cache=False)
            }
            ...
            if all(x is not None for x in [form_3, form_5, form_10]):
                ...
                max_diff = max(abs(form_3 - form_5), abs(form_5 - form_10), abs(form_3 - form_10))
                validations["form_consistency"] = max_diff < 10  # 10 points difference threshold
