...
class FeatureAwarePredictionUtils:
            ...
            if "prob_score" not in features[player_id]:
                try:
                    ...
                    if player:
                        ...
                        if position:
                            df_player = fit_player_data(position, season, gameweek, dbsession=self.dbsession)
                    ...
                    logger.warning(f"Failed to get traditional features for player {player_id}: {e}")
    ...
    def _calc_with_feature_store(
        ...
        if not position or not team:
            raise ValueError(f"Missing position or team for player {player}")
        ...
        if len(recent_minutes) == 0:
            raise ValueError("No recent minutes data available")
        ...
        for fixture in fixtures:
            ...
            if sum(recent_minutes) == 0:
                points = 0.0
            elif player.is_injured_or_suspended(season, min(gw_range), gameweek):
                points = 0.0
            else:
                ...
                for mins in recent_minutes:
                    ...
                    if all(key in features for key in ["prob_score", "prob_assist", "prob_neither"]):
                        ...
                        points += get_attacking_points(position, mins, team_score_prob, player_prob)
                    else:
                        ...
                        points += get_attacking_points(position, mins, team_score_prob, estimated_prob)
    ...
    def batch_update_player_features(
        self,
        season: str = CURRENT_SEASON,
        gameweek_range: Optional[Tuple[int, int]] = None,
        ...
        if position_filter:
            ...
            players = list_players(season=season, gameweek=gameweek_range[1], dbsession=self.dbsession)
        ...
        logger.info(f"Updated features for {len(player_ids)} players in season {season}")
        return stats
...
class FeatureMigrator:
    ...
    def migrate_player_scores_to_timeseries(
        ...
        scores_query = self.dbsession.query(PlayerScore).join(Fixture).filter(
        ...
        for score in scores_query:
            ...
            existing = self.dbsession.query(FeatureTimeSeries).filter_by(
                entity_type="player",
        ...
        logger.info(f"Migrated {migrated_count} time series entries for season {season}")
    ...
    def compute_and_store_historical_features(
        ...
        for season in seasons:
            ...
            stats = self.feature_store.compute_features_batch(
                ...
                entity_ids=player_ids,
                season=season,
                gameweek_range=(6, 38),  # Start from GW6 to have enough history for rolling features
                chunk_size=50
            )
...
class FeatureStoreMonitor:
    ...
    def check_feature_freshness(
        ...
        for feature_name in feature_names:
            feature_def = self.feature_store.registry.get_feature_definition(feature_name)
            
            # Count fresh vs stale features
            fresh_count = self.dbsession.query(ComputedFeature).filter(
                ComputedFeature.feature_definition_id == feature_def.id,
                ComputedFeature.computed_at > cutoff_str
            ).count()
            
            total_count = self.dbsession.query(ComputedFeature).filter(
                ComputedFeature.feature_definition_id == feature_def.id
            ).count()
            ...
            freshness_report[feature_name] = {
                ...
                "is_fresh": fresh_count / total_count >= 0.8 if total_count > 0 else False
            }
        ...
        for feature in features:
            try:
                ...
                if stats["value_stats"]["count"] == 0:
                    report["alerts"].append(f"No computed values for feature {feature.name}")
                
                # Check for feature drift (comparing last two seasons)
                current_season = CURRENT_SEASON
                prev_season = str(int(current_season[:2]) - 1) + str(int(current_season[2:]) - 1)
                ...
                logger.error(f"Failed to generate stats for feature {feature.name}: {e}")
                report["alerts"].append(f"Failed to analyze feature {feature.name}: {str(e)}")
...
def migrate_historical_data(seasons: List[str] = None) -> None:
