...
from airsenal.framework.log_analysis import (
    LogAnalyzer,
...
def analyze_errors(hours_back: int, output_format: str = "json"):
    ...
    if output_format == "json":
        ...
        print(
            f"Functions with Errors: {summary.get('unique_functions_with_errors', 0)}"
        )
...
def analyze_predictions(hours_back: int, output_format: str = "json"):
    ...
    if output_format == "json":
        ...
        print(
            f"Total Prediction Operations: {summary.get('total_prediction_operations', 0)}"
        )
...
def trace_correlation(
    correlation_id: str, hours_back: int, output_format: str = "json"
):
...
def search_log_entries(
    ...
    results = search_logs(
        pattern=pattern, hours_back=hours_back, log_level=level, event_type=event_type
    )
...
Examples:
  ...
  %(prog)s export ./analysis --hours 168
    ...
    parser.add_argument(
        "--hours", "-t", type=int, default=24, help="Hours to look back (default: 24)"
    )
    ...
    subparsers.add_parser("performance", help="Analyze performance metrics")

    # Error analysis
    subparsers.add_parser("errors", help="Analyze error patterns")

    # Prediction analysis
    subparsers.add_parser("predictions", help="Analyze prediction operations")

    # Feature analysis
    subparsers.add_parser("features", help="Analyze feature operations")

    # API analysis
    subparsers.add_parser("api", help="Analyze API call patterns")
    ...
    try:
        if args.command == "performance":
            ...
            search_log_entries(
                args.pattern, args.hours, args.level, args.event_type, args.format
            )
