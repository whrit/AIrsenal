...
def main():
    ...
    try:
        ...
        if args.gameweek:
            ...
            updated_count = manager.sync_xg_data_for_gameweek(args.season, args.gameweek)
            logger.info(f"Updated {updated_count} player scores with xG data")
            
        else:
            logger.error("Please specify --gameweek or use --health-check/--cost-summary")
            return 1
