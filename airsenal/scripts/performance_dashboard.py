...
class PerformanceMetricsCollector:
    ...
    def __init__(self):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self.prediction_latency = Histogram('airsenal_prediction_latency_seconds', 'Prediction latency')
            ...
            self.requests_total = Counter('airsenal_requests_total', 'Total requests', ['endpoint', 'method'])
...
class BenchmarkResultsAnalyzer:
    ...
    def generate_performance_trends(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def create_performance_plots(self, results: List[Dict[str, Any]]) -> List[str]:
        ...
        if timestamps:
            ...
            ax.plot(timestamps, max_durations, label='Max Duration', marker='s', alpha=0.7)
            ax.set_xlabel('Time')
        ...
        if memory_usage and timestamps:
            ax.plot(timestamps[-len(memory_usage):], memory_usage, 
                   label='Peak Memory Usage', marker='o', color='red')
            ax.set_xlabel('Time')
        ...
        if results:
            ...
            if 'results' in latest_result:
                ...
                if categories:
                    ...
                    avg_durations = [sum(durations) / len(durations) for durations in categories.values()]
                    
                    bars = ax.bar(category_names, avg_durations)
...
class PerformanceDashboard:
    ...
    def __init__(self, data_dir: str = "benchmarks/reports", port: int = 8080):
        ...
        if PROMETHEUS_AVAILABLE:
            try:
                ...
                print(f"Prometheus metrics available at http://localhost:{self.port + 1}/metrics")
            except Exception as e:
    ...
    def generate_html_dashboard(self) -> str:
        ...
        recent_alerts = self.metrics_collector.check_alerts(current_metrics)
...
"""
    ...
    def _generate_results_table_html(self, results: List[Dict[str, Any]]) -> str:
        ...
        for result in results:
            summary = result.get('summary', {})
            timestamp = datetime.fromtimestamp(result.get('timestamp', 0)).strftime('%Y-%m-%d %H:%M')
...
def main():
    ...
    parser.add_argument(
        "--generate-only",
        action="store_true",
        help="Generate dashboard once and exit"
    )
    ...
    if args.generate_only:
        ...
        dashboard_file = dashboard.save_dashboard()
        print(f"Dashboard generated: {dashboard_file}")
        print(f"To run continuous monitoring: airsenal_performance_dashboard --monitor")
        if PROMETHEUS_AVAILABLE:
            print(f"Prometheus metrics available at: http://localhost:{args.port + 1}/metrics")
