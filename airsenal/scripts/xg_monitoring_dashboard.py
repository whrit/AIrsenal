    ...
    def print_console_dashboard(self):
        ...
        try:
            ...
            for provider, cost_data in costs["provider_breakdown"].items():
                print(
                    f"  {provider}: ${cost_data['total_cost']:.4f} ({cost_data['requests']} requests)"
                )
            ...
            if recommendations:
                ...
                for rec in recommendations:
                    print(
                        f"  • {rec['provider']} - {rec['type'].title()}: {rec['message']}"
                    )
            else:
    ...
    def generate_alert_config(self, output_file: str = "xg_alerts.json"):
        ...
        alert_config = {
            ...
            "notification_channels": {
                "email": {"type": "email", "recipients": ["admin@example.com"]},
                "slack": {
                    "type": "slack",
...
def main():
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode for web dashboard"
    )
    ...
    try:
        ...
        if args.mode == "alerts":
            dashboard.generate_alert_config()
