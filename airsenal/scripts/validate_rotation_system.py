...
class RotationSystemValidator:
    ...
    def run_full_validation(self) -> dict[str, any]:
        ...
        logger.info("Target accuracy threshold: %.1f%%", self.min_accuracy * 100)
        ...
        try:
            ...
            if len(historical_data) < 100:
                msg = f"Insufficient historical data: {len(historical_data)} events. Need at least 100."
                raise ValueError(msg)

            logger.info("Extracted %d historical rotation events", len(historical_data))
            ...
            self.validation_results = {
                "validation_summary": {
                    ...
                    "meets_threshold": test_metrics.get("accuracy", 0.0)
                    >= self.min_accuracy,
    ...
    def _extract_historical_rotation_events(self) -> pd.DataFrame:
        ...
        for season in training_seasons:
            ...
            for team in teams:
                if self.verbose:
                    logger.info("Processing team %s...", team)
    ...
    def _extract_team_rotation_events(
        self, team: str, season: str, fixtures: list[Fixture]
    ) -> list[dict]:
        ...
        for fixture in team_fixtures[:30]:  # Limit to first 30 fixtures for performance
            try:
                ...
                if self.verbose:
                    logger.warning(
                        "Error processing fixture %s: %s", fixture.fixture_id, e
                    )
                continue

        return events

    def _calculate_historical_risk_factors(
        ...
        for _, event in historical_data.iterrows():
            try:
                ...
                if self.verbose:
                    logger.warning(
                        "Error calculating factors for player %s: %s",
                        event["player_id"],
                        e,
                    )
                continue
        ...
        logger.info(
            "Successfully calculated risk factors for %d events", len(result_df)
        )
    ...
    def _prepare_datasets(
        ...
        if len(training_data) < min_training_size:
            ...
            training_data, test_data = train_test_split(
                ...
                test_size=0.3,
                random_state=42,
                stratify=labeled_data["was_rotated"],
            )

        logger.info("Training set: %d events", len(training_data))
        logger.info("Test set: %d events", len(test_data))
        logger.info(
            "Training rotation rate: %.1f%%", training_data["was_rotated"].mean() * 100
        )
        logger.info("Test rotation rate: %.1f%%", test_data["was_rotated"].mean() * 100)
    ...
    def _train_models(self) -> dict[str, any]:
        ...
        try:
            ...
            logger.info(
                ...
                training_metrics.get("forest_accuracy", 0),
            )
            logger.info(
                "Logistic Regression accuracy: %.3f",
                training_metrics.get("logistic_accuracy", 0),
            )
    ...
    def _validate_on_test_set(self) -> dict[str, any]:
        ...
        try:
            if not self.calculator.is_trained:
                msg = "Model must be trained before validation"
                raise ValueError(msg)
            ...
            test_metrics.update(
                {
                    ...
                    "forest_classification_report": classification_report(
                        y_test, forest_pred, output_dict=True
                    ),
                    "logistic_classification_report": classification_report(
                        y_test, logistic_pred, output_dict=True
                    ),
                    "confusion_matrix_forest": confusion_matrix(
                        y_test, forest_pred
                    ).tolist(),
                    "confusion_matrix_logistic": confusion_matrix(
            ...
            test_metrics["accuracy"] = max(
                test_metrics["forest_accuracy"], test_metrics["logistic_accuracy"]
            )

            logger.info("Test set validation completed")
            logger.info("Best model accuracy: %.3f", test_metrics["accuracy"])
            logger.info(
                "Meets %.1f%% threshold: %s",
                self.min_accuracy * 100,
                test_metrics["accuracy"] >= self.min_accuracy,
            )

            return test_metrics

        except Exception as e:
            logger.error("Test validation failed: %s", e)
    ...
    def _cross_validation_analysis(self) -> dict[str, any]:
        ...
        try:
            ...
            forest_cv_scores = cross_val_score(
                self.calculator.forest_model, X, y, cv=5, scoring="accuracy"
            )
            logistic_cv_scores = cross_val_score(
                self.calculator.logistic_model, X, y, cv=5, scoring="accuracy"
            )
            ...
            cv_metrics = {
                ...
                "best_cv_accuracy": max(
                    forest_cv_scores.mean(), logistic_cv_scores.mean()
                ),
            }

            logger.info("Cross-validation completed")
            logger.info(
                "Forest CV accuracy: %.3f ± %.3f",
                cv_metrics["forest_cv_mean"],
                cv_metrics["forest_cv_std"],
            )
            logger.info(
                "Logistic CV accuracy: %.3f ± %.3f",
                cv_metrics["logistic_cv_mean"],
                cv_metrics["logistic_cv_std"],
            )
        ...
        try:
            ...
            segment_analysis["by_player_importance"] = {
                "high_importance_players": {
                    ...
                    "rotation_rate": high_importance["was_rotated"].mean()
                    if len(high_importance) > 0
                    else 0,
                },
                "low_importance_players": {
                    "count": len(low_importance),
                    "rotation_rate": low_importance["was_rotated"].mean()
            ...
            high_congestion = self.test_data[
                self.test_data["fixture_congestion"] >= 0.6
            ]
            ...
            segment_analysis["by_fixture_congestion"] = {
                "high_congestion": {
                    ...
                    "rotation_rate": high_congestion["was_rotated"].mean()
                    if len(high_congestion) > 0
                    else 0,
                },
                "low_congestion": {
                    "count": len(low_congestion),
                    "rotation_rate": low_congestion["was_rotated"].mean()
                    if len(low_congestion) > 0
                    else 0,
                },
            }
            ...
            logger.error("Segment analysis failed: %s", e)
            return {"error": str(e)}
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        test_accuracy = self.validation_results.get("test_metrics", {}).get(
            "accuracy", 0
        )

        if test_accuracy >= self.min_accuracy:
            recommendations.append(
                f"✅ Model meets accuracy threshold ({test_accuracy:.1%} >= {self.min_accuracy:.1%})"
            )
            recommendations.append("Model is ready for production use")
        else:
            recommendations.append(
                f"❌ Model does not meet accuracy threshold ({test_accuracy:.1%} < {self.min_accuracy:.1%})"
            )
            recommendations.append("Consider the following improvements:")

            # Feature-specific recommendations
            feature_importance = self.validation_results.get(
                "feature_importance", {}
            ).get("feature_importance_dict", {})
            if feature_importance:
                least_important = min(feature_importance.items(), key=lambda x: x[1])
                recommendations.append(
                    f"- Remove or improve low-importance feature: {least_important[0]}"
                )

            # Data-specific recommendations
            training_size = self.validation_results.get("validation_summary", {}).get(
                "training_events", 0
            )
            if training_size < 200:
                recommendations.append(
                    "- Collect more historical data for training (current: {training_size} events)"
                )
        ...
        cv_std = self.validation_results.get("cross_validation", {}).get(
            "forest_cv_std", 0
        )
        if cv_std > 0.1:
            recommendations.append(
                "- Model shows high variance; consider regularization"
            )
    ...
    def _log_validation_results(self):
        ...
        self.validation_results.get("test_metrics", {})

        logger.info("=" * 60)
        logger.info("ROTATION RISK SYSTEM VALIDATION RESULTS")
        logger.info("=" * 60)
        logger.info("Total historical events: %d", summary.get("total_events", 0))
        logger.info("Training events: %d", summary.get("training_events", 0))
        logger.info("Test events: %d", summary.get("test_events", 0))
        logger.info("Target accuracy: %.1f%%", summary.get("target_accuracy", 0) * 100)
        logger.info(
            "Achieved accuracy: %.1f%%", summary.get("achieved_accuracy", 0) * 100
        )
        logger.info("Meets threshold: %s", summary.get("meets_threshold", False))
        logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate rotation risk prediction system"
    )
    parser.add_argument(
        "--season", default=None, help="Season to validate (default: current season)"
    )
    parser.add_argument(
        "--min-accuracy",
        type=float,
        default=0.8,
        help="Minimum required accuracy (default: 0.8)",
    )
    ...
    validator = RotationSystemValidator(
        season=args.season, min_accuracy=args.min_accuracy, verbose=args.verbose
    )
