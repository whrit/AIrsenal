...
def main():
    ...
    operation_group.add_argument(
        "--seasons", nargs="+", help="Seasons to backfill (e.g., 2122 2223 2324)"
    )

    operation_group.add_argument(
        "--resume", type=str, help="Resume backfill from session ID"
    )
    ...
    parser.add_argument(
        "--gameweeks", nargs="+", type=int, help="Specific gameweeks to backfill"
    )
    ...
    try:
        ...
        if not xg_manager.providers:
            ...
            print(
                "Please ensure you have a valid API key for Sportmonks or other providers."
            )
            return 1
        ...
        if args.incremental:
            return handle_incremental_backfill(
                backfiller, args.season, args, output_dir
            )
        if args.missing_data_report:
            return handle_missing_data_report(backfiller, args.season, output_dir)
        if args.validate_only:
            return handle_validation_only(
                backfiller, args.season, args.gameweeks, output_dir
            )
        if args.gameweeks:
            return handle_gameweeks_backfill(
                backfiller, args.season, args.gameweeks, args, output_dir
            )
...
def handle_seasons_backfill(
    backfiller: HistoricalDataBackfiller, seasons: list[str], args, output_dir: Path
) -> int:
    ...
    for i, season in enumerate(seasons, 1):
        ...
        print(
            f"{status_emoji} Season {season}: {result.status.value} "
    ...
    failed_seasons = [
        s for s, r in results.items() if r.status == BackfillStatus.FAILED
    ]
...
def handle_gameweeks_backfill(
    ...
    status_emoji = "✅" if result.status == BackfillStatus.COMPLETED else "❌"
    print(
        f"{status_emoji} Gameweeks backfill: {result.status.value} "
        f"({result.progress.completed_items}/{result.progress.total_items} gameweeks, "
        f"{duration:.1f}s)"
    )
...
def handle_resume_backfill(
    backfiller: HistoricalDataBackfiller, session_id: str, args, output_dir: Path
) -> int:
    ...
    if session_id.startswith("season_"):
        ...
        if len(parts) >= 3:
            ...
            status_emoji = "✅" if result.status == BackfillStatus.COMPLETED else "❌"
            print(
                f"{status_emoji} Resumed backfill: {result.status.value} ({duration:.1f}s)"
            )

            # Save report
            report_path = output_dir / f"resumed_backfill_{session_id}.json"
            save_result_report(
                result, report_path, {"session_id": session_id, "season": season}
            )
...
def handle_incremental_backfill(
    backfiller: HistoricalDataBackfiller, season: str, args, output_dir: Path
) -> int:
    ...
    status_emoji = "✅" if result.status == BackfillStatus.COMPLETED else "❌"
    print(
        f"{status_emoji} Incremental backfill: {result.status.value} ({duration:.1f}s)"
    )
    ...
    save_result_report(
        result,
        report_path,
        {"season": season, "incremental_days": args.incremental_days},
    )

    return 0 if result.status == BackfillStatus.COMPLETED else 1


def handle_missing_data_report(
    backfiller: HistoricalDataBackfiller, season: str, output_dir: Path
) -> int:
    """Generate missing data report"""
...
def generate_summary_report(
    results: dict[str, any], output_dir: Path, operation_type: str
):
    ...
    summary = {
        ...
        "successful_operations": len(
            [r for r in results.values() if r.status == BackfillStatus.COMPLETED]
        ),
        "failed_operations": len(
            [r for r in results.values() if r.status == BackfillStatus.FAILED]
        ),
        "results": {},
    }
...
def save_result_report(result, report_path: Path, metadata: dict | None = None):
