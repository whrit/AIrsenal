...
def create_mock_player_scores(num_scores: int) -> list[tuple[PlayerScore, str]]:
    ...
    for i in range(num_scores):
        ...
        score.assists = max(
            0, int(np.random.poisson(0.3))
        )  # Average ~0.3 assists per game
        score.bonus = max(0, int(np.random.poisson(0.8)))  # Average ~0.8 bonus per game
        score.clean_sheets = np.random.choice(
            [0, 1], p=[0.65, 0.35]
        )  # ~35% clean sheet rate
        score.saves = max(
            0, int(np.random.poisson(3.0))
        )  # Goalkeepers average ~3 saves
        score.own_goals = np.random.choice([0, 1], p=[0.99, 0.01])  # ~1% own goal rate
        score.penalties_saved = np.random.choice(
            [0, 1], p=[0.98, 0.02]
        )  # ~2% penalty save rate
        score.minutes = np.random.choice(
            [0, 90, 45, 30], p=[0.1, 0.7, 0.15, 0.05]
        ...
        position = (
            positions[i % 4]
            if i < 4
            else np.random.choice(positions, p=[0.05, 0.35, 0.35, 0.25])
        )
...
def benchmark_batch_calculations(
    ...
    for run in range(num_runs):
                ...
                base_metrics.update(
                    {
                        ...
                        "penalty_taker_boost": np.random.choice(
                            [0.0, 1.0], p=[0.8, 0.2]
                        ),
                    }
                )
        ...
        try:
            ...
            results = calculator.calculate_batch_scores(
                scores_and_positions, apply_difficulty_adjustment=False
            )
...
def benchmark_individual_calculations(
    calculator: WeightedPerformanceCalculator,
    ...
    try:
        ...
        for _ in range(num_calculations):
            result = calculator.calculate_weighted_score(
                score, "DEF", apply_difficulty_adjustment=False
            )
...
def benchmark_memory_usage(
    calculator: WeightedPerformanceCalculator, num_players: int
) -> dict:
    """
    ...
    print(
        f"  Reasonable usage (<100MB): {'✓ PASSED' if results['reasonable_usage'] else '✗ FAILED'}"
    )
...
def validate_accuracy(calculator: WeightedPerformanceCalculator) -> dict:
    ...
    try:
        ...
        batch_results = calculator.calculate_batch_scores(
            scores_and_positions, apply_difficulty_adjustment=False
        )
        individual_results = [
        ...
        batch_consistent = all(
            abs(batch_results[i] - individual_results[i]) < 1e-10 for i in range(5)
        )
    ...
    print("\nResults:")
    print(
        f"  Individual consistency: {'✓ PASSED' if results['consistent_individual'] else '✗ FAILED'}"
    )
    print(
        f"  Batch vs individual: {'✓ PASSED' if results['consistent_batch_vs_individual'] else '✗ FAILED'}"
    )
    print(
        f"  Result in range [0,1]: {'✓ PASSED' if results['result_in_range'] else '✗ FAILED'}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate weighted performance metrics system"
    )
    parser.add_argument(
        "--num-players",
        type=int,
    ...
    if batch_results["target_met"]:
        ...
        print(
            f"✗ Batch calculation performance: FAILED ({batch_results['max_time_ms']:.2f}ms)"
        )
        all_passed = False
    ...
    if individual_results["target_met"]:
        ...
        print(
            f"✗ Individual calculation performance: FAILED ({individual_results['avg_time_ms']:.4f}ms avg)"
        )
        all_passed = False
    ...
    # Check accuracy
    accuracy_passed = (
        accuracy_results["consistent_individual"]
        and accuracy_results["consistent_batch_vs_individual"]
        and accuracy_results["result_in_range"]
    )
    if accuracy_passed:
