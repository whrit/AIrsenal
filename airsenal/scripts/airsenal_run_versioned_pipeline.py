...
from airsenal.framework.versioned_prediction_utils import (
    ...
    make_versioned_predictedscore_table,
)
...
@click.option("--fpl_team_id", type=int, required=False, help="fpl team id for pipeline run")
@click.option("--clean", is_flag=True, help="If set, delete and recreate the AIrsenal database")
@click.option("--apply_transfers", is_flag=True, help="If set, go ahead and make the transfers via the API.")

# Model versioning options
@click.option("--use-versioned-models", is_flag=True, help="Use model versioning system")
@click.option("--use-production-models", is_flag=True, help="Use production models")
@click.option("--player-model-versions", help="JSON string of player model versions (e.g., {'fwd': 'v1.0.0'})")
@click.option("--team-model-version", help="Specific team model version to use")
@click.option("--train-and-register", is_flag=True, help="Train new models and register them")
@click.option("--model-version", help="Version string for newly trained models")

# A/B testing options
@click.option("--ab-test", nargs=2, metavar=("CONTROL_VERSION", "TREATMENT_VERSION"), 
              help="Run A/B test with control and treatment versions")
@click.option("--experiment-name", help="Name for the A/B test experiment")
@click.option("--traffic-split", type=float, default=0.5, help="Traffic split for A/B test")

# Standard pipeline options
@click.option("--wildcard_week", type=int, help="Play wildcard in specified week", default=-1)
@click.option("--free_hit_week", type=int, help="Play free hit in specified week", default=-1)
@click.option("--triple_captain_week", type=int, help="Play triple captain in specified week", default=-1)
@click.option("--bench_boost_week", type=int, help="Play bench boost in specified week", default=-1)
@click.option("--n_previous", help="Number of past seasons to include", type=int, default=3)
@click.option("--no_current_season", help="Exclude current season from database", is_flag=True)
...
def main(
    ...
    with session_scope() as session:
        ...
        try:
            ...
            do_attributes = True
            update_db(season, do_attributes, fpl_team_id or fetcher.FPL_TEAM_ID, session)
            ...
            if ab_test:
                ...
                experiment_tag = _run_ab_test(
                    session, gw_range, season, control_version, treatment_version,
                    experiment_name, traffic_split
                )
                ...
                tag = _run_versioned_predictions(
                    session, gw_range, season, use_production_models,
                    player_model_versions, team_model_version
                )
