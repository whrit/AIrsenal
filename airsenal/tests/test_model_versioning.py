...
from airsenal.framework.ab_testing import (
    ABTestManager,
    ExperimentConfig,
    ExperimentStatus,
)
...
def mock_player_model():
    ...
    model.mean_probabilities = np.array(
...
class TestModelArtifactManager:
    ...
    def test_deserialize_conjugate_model(self, mock_player_model):
        ...
        assert np.array_equal(
            deserialized.mean_probabilities, mock_player_model.mean_probabilities
        )
    ...
    def test_save_and_load_artifact(
        self, test_config, test_db_session, mock_player_model
    ):
...
class TestModelVersionManager:
    ...
    def test_register_model_version(
        ...
        artifacts = (
            test_db_session.query(ModelArtifact)
            .filter_by(version_id=model_version.id)
            .all()
        )
    ...
    def test_load_latest_model(self, test_config, test_db_session, mock_player_model):
        ...
        manager.load_model_version("test_model")

        # Verify it's the latest version by checking the database
        latest_version = (
            test_db_session.query(ModelVersion)
            .join(ModelRegistry)
            .filter(ModelRegistry.model_name == "test_model")
    ...
    def test_cleanup_old_versions(
        ...
        deleted_count = manager.cleanup_old_versions(
            "test_model", keep_latest=3, keep_production=True
        )
...
class TestABTestManager:
    ...
    @patch(
        "airsenal.framework.versioned_prediction_utils.VersionedPredictionManager.run_versioned_predictions"
    )
    def test_run_experiment_iteration(
        self, mock_run_predictions, test_db_session, mock_player_model
    ):
    ...
    def test_analyze_experiment_results(self, test_db_session, mock_player_model):
        ...
        with patch.object(
            ab_manager, "_get_predictions_by_tags"
...
class TestVersionedPredictionManager:
    ...
    def test_fit_and_register_player_model(self, mock_process_data, test_db_session):
        ...
        registry = (
            test_db_session.query(ModelRegistry)
            .filter_by(model_name="player_model_fwd")
            .first()
        )
        ...
        version_record = (
            test_db_session.query(ModelVersion)
            .filter_by(registry_id=registry.id, version="1.0.0")
            .first()
        )
...
class TestConvenienceFunctions:
    ...
    def test_register_model_function(self, mock_player_model):
        ...
        with patch(
            "airsenal.framework.model_versioning.ModelVersionManager"
    ...
    def test_load_model_function(self):
        ...
        with patch(
            ...
            mock_manager.load_model_version.assert_called_once_with(
                "test_model", "1.0.0"
            )
...
class TestIntegration:
    ...
    def test_end_to_end_workflow(self, test_config, test_db_session, mock_player_model):
        ...
        manager.register_model_version(
            mock_player_model, "test_model", "1.0.0", performance_metrics={"mae": 0.20}
        )
        manager.register_model_version(
            mock_player_model, "test_model", "1.1.0", performance_metrics={"mae": 0.18}
        )
        ...
        manager.load_model_version("test_model", "1.1.0")
        prod_model = manager.get_production_model("test_model")

        # 5. Compare models
        comparison = manager.compare_models(
            "test_model", ["1.0.0", "1.1.0"], ["validation_mae"]
        )

        # Verify the workflow
        assert len(comparison) == 2
        assert (
            comparison.loc[1, "validation_mae"] < comparison.loc[0, "validation_mae"]
        )  # v1.1.0 is better
        assert experiment.status == ExperimentStatus.PLANNED.value
