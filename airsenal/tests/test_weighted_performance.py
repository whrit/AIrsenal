...
class TestWeightedPerformanceCalculator:
    ...
    def test_calculate_weighted_score_basic(self, mock_calculator):
        ...
        with patch.object(calculator, "extract_metrics_from_score") as mock_extract:
            ...
            score_result = calculator.calculate_weighted_score(
...
class TestConvenienceFunctions:
    ...
    def test_calculate_weighted_performance(self):
        ...
        with patch(
            "airsenal.framework.weighted_performance.default_calculator"
        ) as mock_calc:
    ...
    def test_calculate_batch_weighted_performance(self):
        ...
        with patch(
            ...
            assert len(result) == 2
            mock_calc.calculate_batch_scores.assert_called_once_with(
                scores_and_positions
            )
...
class TestPerformanceBenchmarking:
    ...
    def test_batch_calculation_performance(self):
        ...
        with patch.object(
            calculator, "extract_metrics_from_score", side_effect=mock_extract_metrics
        ):
            results = calculator.calculate_batch_scores(
                scores_and_positions, apply_difficulty_adjustment=False
            )
        ...
        print(
            f"Batch calculation time for {num_scores} scores: {calculation_time_ms:.2f}ms"
        )
        assert calculation_time_ms < 50.0, (
            f"Calculation took {calculation_time_ms:.2f}ms, exceeds 50ms target"
        )
    ...
    def test_individual_calculation_performance(self):
        ...
        with patch.object(calculator, "extract_metrics_from_score") as mock_extract:
            ...
            assert avg_time_ms < 1.0, (
                f"Individual calculation averaging {avg_time_ms:.4f}ms is too slow"
            )
