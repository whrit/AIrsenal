...
class TestFormCalculator:
    ...
    def calculator_with_cache(self, mock_session):
        ...
        with patch(
            "airsenal.framework.form_calculator.FeatureStore",
            return_value=mock_feature_store,
        ):
            return FormCalculator(dbsession=mock_session, enable_caching=True)
    ...
    def test_calculate_weighted_form_custom_decay(self, calculator, mock_query_result):
        ...
        custom_decay = 0.8
        weighted_form = calculator.calculate_weighted_form(
            player_id=123, gameweeks=3, decay_factor=custom_decay
        )
    ...
    def test_batch_calculate_form_success(self, calculator, mock_query_result):
        ...
        for _player_id, metrics in results.items():
    ...
    def test_batch_calculate_form_no_momentum(self, calculator, mock_query_result):
        ...
        player_ids = [123, 456]
        results = calculator.batch_calculate_form(
            player_ids, include_momentum=False, use_cache=False
        )
    ...
    def test_batch_calculate_form_chunking(self, calculator, mock_query_result):
        ...
        results = calculator.batch_calculate_form(
            player_ids, chunk_size=3, use_cache=False
        )
    ...
    def test_performance_requirement(self, calculator, mock_query_result):
        ...
        form = calculator.calculate_rolling_form(
            player_id=123, gameweeks=5, use_cache=False
        )
    ...
    def test_caching_functionality(self, calculator_with_cache, mock_query_result):
        ...
        calculator_with_cache.calculate_rolling_form(
            player_id=123, gameweeks=3, use_cache=True
        )
        ...
        form2 = calculator_with_cache.calculate_rolling_form(
            player_id=123, gameweeks=3, use_cache=True
        )
    ...
    def test_update_player_attributes_success(self, calculator):
        ...
        with (
            patch.object(calculator, "calculate_rolling_form") as mock_rolling,
    ...
    def test_error_handling_database_error(self, calculator):
        ...
        with pytest.raises(FormCalculationError):
            calculator.calculate_rolling_form(
                player_id=123, gameweeks=3, use_cache=False
            )
    ...
    def test_error_handling_invalid_parameters(self, calculator, mock_query_result):
        ...
        with pytest.raises((ValueError, FormCalculationError)):
            calculator.calculate_weighted_form(
                player_id=123, gameweeks=5, decay_factor=1.5
            )
...
class TestFormCalculatorPerformance:
    ...
    @pytest.fixture
    def calculator(self):
        """Create a FormCalculator instance for performance tests."""
    ...
    def mock_query_result(self):
        """Mock database query result for performance tests."""
        # Create sample player score data for testing
        gameweeks = list(range(15, 0, -1))  # GW 15 to GW 1 (descending order)
        points = [8, 12, 4, 15, 6, 9, 11, 2, 14, 7, 5, 13, 10, 3, 16]
        ...
        for i, gw in enumerate(gameweeks):
            ...
            score.gameweek = gw
            score.points = points[i]
            score.goals = 1 if i % 3 == 0 else 0
            score.assists = 1 if i % 4 == 0 else 0
            score.minutes = 90
            score.opponent = f"Team_{i % 5}"
            score.date = f"2024-{gw:02d}-01"
            scores.append(score)
        ...
        mock_query = Mock()
        mock_query.all.return_value = scores
        return mock_query
    ...
    def test_batch_processing_performance(self, calculator, mock_query_result):
        ...
        start_time = time.time()
        results = calculator.batch_calculate_form(
            player_ids, use_cache=False, chunk_size=20
        )
