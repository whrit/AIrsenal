...
class TestTrendDetector:
    ...
    def sample_performance_data(self):
        ...
        data = []
        for i, (gw, date, pts) in enumerate(
            zip(gameweeks, dates, points, strict=False)
        ):
            pts_value = max(0, pts)  # Ensure non-negative points
            data.append(
                {
                    "gameweek": gw,
                    "points": pts_value,
                    "adjusted_points": pts_value,  # Add adjusted_points column
    ...
    def declining_trend_data(self):
        ...
        # Declining trend - make it more pronounced for statistical significance
        base_points = 8.0
        trend_slope = -0.5  # Steeper decline
        noise = np.random.normal(0, 0.5, 15)  # Less noise
    ...
    def test_initialization(self, mock_session):
        ...
        assert not detector.seasonal_adjustment
        assert not detector.weight_recent_data
    ...
    def test_sens_slope_estimator(self, trend_detector):
        ...
        )  # Allow equal bounds for perfect data
    ...
    @patch(
        "airsenal.framework.trend_detection.TrendDetector._get_player_performance_data"
    )
    def test_detect_trends_improving(
        self, mock_get_data, trend_detector, sample_performance_data
    ):
    ...
    def test_detect_trends_declining(
        self, mock_get_data, trend_detector, declining_trend_data
    ):
    ...
    def test_detect_trends_insufficient_data(self, trend_detector):
        ...
        with patch.object(
            trend_detector, "_get_player_performance_data"
        ) as mock_get_data:
            # Mock insufficient data
            pd.DataFrame(
                {"gameweek": [1, 2], "points": [5, 6], "adjusted_points": [5, 6]}
            )
...
class TestChangePointDetector:
    ...
    def change_point_data(self):
        ...
        df_data = []
        for i, (gw, pts) in enumerate(zip(gameweeks, data, strict=False)):
            pts_value = max(0, pts)
            df_data.append(
                {
                    "gameweek": gw,
                    "points": pts_value,
                    "adjusted_points": pts_value,  # Ensure both columns have same value
                    "date": dates[i],
                }
            )
    ...
    def test_initialization(self, mock_session):
        ...
        detector = ChangePointDetector(
            dbsession=mock_session, min_segment_length=3, penalty=2.0, model="poisson"
        )
    ...
    def test_binary_segmentation(self, change_point_detector):
        ...
        change_points = change_point_detector._binary_segmentation(
            data, max_change_points=3
        )
    ...
    def test_find_breakpoints(
        self, change_point_detector, change_point_data
    ):
        """Test change point detection with real data."""
        df, true_change_points = change_point_data
        change_point_detector.trend_detector._get_player_performance_data.return_value = df
    ...
    def test_find_breakpoints_binary_segmentation(
        self, change_point_detector, change_point_data
    ):
    ...
    def test_find_breakpoints_insufficient_data(self, change_point_detector):
        ...
        with patch.object(
            change_point_detector.trend_detector, "_get_player_performance_data"
        ) as mock_get_data:
            mock_get_data.side_effect = InsufficientDataError("Insufficient data")
...
class TestTrendVisualizer:
    ...
    @pytest.mark.skipif(
        not PLOTTING_AVAILABLE, reason="matplotlib/seaborn not available"
    )
    def test_plot_trend_analysis(
        self, trend_visualizer, sample_trend_result, sample_change_points
    ):
    ...
    def test_plot_trend_analysis_no_plotting(self, sample_trend_result):
        ...
        with patch("airsenal.framework.trend_detection.PLOTTING_AVAILABLE", False):
            ...
            result = visualizer.plot_trend_analysis(
                player_id=123, trend_result=sample_trend_result, show_plot=False
            )
    ...
    def test_plot_batch_trends(self, trend_visualizer, sample_trend_result):
        ...
        result = trend_visualizer.plot_batch_trends(
            trend_results=trend_results, show_plot=False
        )
    ...
    def test_plot_batch_trends_empty(self, trend_visualizer):
        ...
        result = trend_visualizer.plot_batch_trends(trend_results={}, show_plot=False)
...
class TestAlertSystem:
    ...
    def test_check_change_point_alerts(
        self, alert_system, high_confidence_change_points
    ):
    ...
    def test_check_significant_changes(
        self,
        alert_system,
        improving_trend_result,
        high_confidence_change_points,
    ):
        """Test comprehensive significant change checking."""
        # Mock batch detection results
        alert_system.trend_detector.batch_detect_trends.return_value = {
            123: improving_trend_result
        }
        alert_system.change_point_detector.batch_find_breakpoints.return_value = {
            123: high_confidence_change_points
        }
...
class TestTrendResult:
    ...
    def test_is_significant(self, trend_result):
        ...
        assert trend_result.is_significant(alpha=0.05)
        assert not trend_result.is_significant(alpha=0.01)
...
class TestAccuracyValidation:
    ...
    def test_trend_detection_accuracy_synthetic_data(self, trend_detector):
        ...
        # Strong upward trend - make signal stronger relative to noise
        upward_trend = np.array(
            [1 + 0.8 * i + np.random.normal(0, 0.1) for i in range(20)]
        )
        test_cases.append((upward_trend, TrendDirection.IMPROVING, "strong_upward"))

        # Strong downward trend - make signal stronger relative to noise
        downward_trend = np.array(
            [10 - 0.6 * i + np.random.normal(0, 0.1) for i in range(20)]
        )
        test_cases.append((downward_trend, TrendDirection.DECLINING, "strong_downward"))

        # No trend (stable) - small noise around constant value
        stable = np.array([5 + np.random.normal(0, 0.3) for i in range(20)])
        ...
        for data, expected_direction, case_name in test_cases:
            ...
            with patch.object(
                trend_detector, "_get_player_performance_data", return_value=mock_df
            ):
                try:
                    result = trend_detector.detect_trends(
                        player_id=123, lookback_days=30
                    )
        ...
        # Target: 66% accuracy (2 out of 3 test cases)
        # Note: The Mann-Kendall test is conservative and the stable case 
        # might be classified as UNKNOWN rather than STABLE
        assert accuracy >= 0.66, f"Accuracy {accuracy:.2%} below target 66%"
