...
class ModelVersionFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    config_hash = factory.LazyFunction(
        lambda: hashlib.sha256(
            f"config_{random.randint(1000, 9999)}".encode()
        ).hexdigest()[:16]
    )
    ...
    cross_validation_score = factory.LazyAttribute(
        lambda obj: obj.validation_accuracy - random.uniform(0.02, 0.08)
        if obj.validation_accuracy
        else None
    )
    ...
    status = factory.fuzzy.FuzzyChoice(
        [
            ...
            "ready",
            "ready",
            "ready",  # Most models should be ready
            "training",
            "deployed",
            "deprecated",
            "failed",
...
class ModelArtifactFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    checksum = factory.LazyFunction(
        lambda: hashlib.sha256(
            f"artifact_{random.randint(10000, 99999)}".encode()
        ).hexdigest()
    )

    compression = factory.fuzzy.FuzzyChoice(
        ["gzip", "bzip2", "none"], weights=[3, 1, 2]
    )
...
class ModelPerformanceFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    rmse = factory.LazyAttribute(lambda obj: obj.mae * random.uniform(1.2, 1.8))
    ...
    recall = factory.LazyAttribute(
        lambda obj: obj.precision + random.uniform(-0.03, 0.03)
        if obj.precision
        else None
    )

    f1_score = factory.LazyAttribute(
        lambda obj: _calculate_f1_score(obj.precision, obj.recall)
        if obj.precision and obj.recall
        else None
    )
    ...
    top_transfer_accuracy = factory.LazyAttribute(
        lambda obj: random.uniform(0.4, 0.8)
        if "transfer" in obj.version.registry.model_name
        else None
    )

    points_captured = factory.LazyAttribute(
        lambda obj: random.uniform(0.6, 0.95)
        if "player" in obj.version.registry.model_name
        else None
    )
    ...
    additional_metrics = factory.LazyAttribute(
        lambda obj: json.dumps(
            _generate_additional_metrics(obj.version.registry.model_name)
        )
    )
...
class ModelExperimentFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    status = factory.fuzzy.FuzzyChoice(
        ["planned", "running", "completed", "stopped"], weights=[1, 3, 2, 1]
    )
...
def _generate_accuracy_for_model(model_type: str) -> float:
    ...
    if any(
        ...
        for keyword in ["classification", "selector", "predictor"]
    ):
...
def _get_serialization_format(model_type: str) -> str:
    ...
    if "Numpyro" in model_type or "Jax" in model_type:
        return "jax"
    if "Genetic" in model_type or "Optimizer" in model_type:
        return "pickle"
    if "DixonColes" in model_type:
        return "joblib"
    return "pickle"
...
def create_model_family(
    session, model_name: str, num_versions: int = 5
) -> list[ModelVersion]:
    ...
    for _i in range(num_versions):
        ...
        for artifact_type in ["full_model", "metadata"]:
            ModelArtifactFactory(
                session=session, version=version, artifact_type=artifact_type
            )

        # Create performance records
        for dataset_type in ["validation", "test"]:
            ModelPerformanceFactory(
                session=session, version=version, dataset_type=dataset_type
            )
...
def create_ab_test_scenario(session) -> dict[str, Any]:
    ...
    control_registry = ModelRegistryFactory(
        session=session, model_name="player_model_mid"
    )
    control_version = ProductionModelVersionFactory(
        session=session, registry=control_registry, version="2.1.0"
    )

    # Create treatment model (experimental)
    treatment_version = ModelVersionFactory(
        session=session, registry=control_registry, version="2.2.0-beta"
    )
    ...
    for version in [control_version, treatment_version]:
        ModelPerformanceFactory(
            session=session, version=version, dataset_type="production"
        )
...
def create_model_development_pipeline(session) -> dict[str, list[ModelVersion]]:
    ...
    dev_versions = [
        ModelVersionFactory(
            session=session, registry=registry, status="training", version="3.0.0-dev"
        ),
        ModelVersionFactory(
            session=session, registry=registry, status="ready", version="3.0.0-alpha"
        ),
    ]

    # Staging versions
    staging_versions = [
        BetaModelVersionFactory(
            session=session, registry=registry, version="3.0.0-beta"
        ),
        ModelVersionFactory(
            session=session, registry=registry, status="ready", version="3.0.0-rc1"
        ),
    ]
    ...
    production_versions = [
        ProductionModelVersionFactory(
            session=session, registry=registry, version="2.5.0"
        ),
        ProductionModelVersionFactory(
            session=session, registry=registry, version="2.4.1", status="deprecated"
        ),
    ]
