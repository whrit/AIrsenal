...
from .database_factories import (
    ...
    create_failed_migration_scenario,
...
class TestDatabaseManager:
    ...
    def seed_season_data(
        self, season: str = "2324", num_gameweeks: int = 10
    ) -> dict[str, Any]:
        """
        ...
        feature_timelines = {}
        for _i, player_attrs in enumerate(all_players[:20]):  # Top 20 players
            timeline = create_player_feature_timeline(
                session, player_id=player_attrs.player_id, season=season
            )
        ...
        self._seeded_data.update(seeded_data)
        logger.info(
            f"Seeded season data: {season}, {num_gameweeks} gameweeks, {len(all_players)} player records"
        )
        return seeded_data
    ...
    def seed_model_versioning_data(self) -> dict[str, Any]:
        ...
        self._seeded_data.update(seeded_data)
        logger.info(
            f"Seeded model versioning data: {len(model_names)} model families, {len(ab_tests)} A/B tests"
        )
        return seeded_data
    ...
    def seed_database_versioning_data(self) -> dict[str, Any]:
        ...
        self._seeded_data.update(seeded_data)
        logger.info(
            f"Seeded database versioning data: {len(migration_timeline)} versions, {len(compatibility_matrix)} compatibility entries"
        )
        return seeded_data
    ...
    def get_seeded_data(self, data_type: str | None = None) -> dict[str, Any]:
        """
    ...
    def verify_data_integrity(self) -> dict[str, Any]:
        ...
        players_with_attrs = session.execute(
            text("""
        ...
        models_with_versions = session.execute(
            text("""
...
def create_test_database(
    preset: str = "standard", database_url: str | None = None
) -> TestDatabaseManager:
    """
...
class TestDatabaseContext:
    ...
    def __init__(self, preset: str = "standard", database_url: str | None = None):
        self.preset = preset
