...
class FeatureDefinitionFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    name = factory.fuzzy.FuzzyChoice([
        # Player features
        "rolling_goals_5", "rolling_assists_5", "rolling_points_5",
        "xg_form_3", "xa_form_3", "xgi_form_3",
        "minutes_per_game_5", "bonus_points_rate",
        "home_away_split", "big_team_performance",
        "fixture_difficulty_weighted_form",
        "penalty_conversion_rate", "set_piece_involvement",
        
        # Team features  
        "team_attack_strength", "team_defense_strength",
        "team_goals_scored_5", "team_goals_conceded_5",
        "team_clean_sheet_rate", "team_home_advantage",
        "team_recent_form_5", "team_injury_impact",
        
        # Fixture features
        "fixture_importance", "derby_match_indicator",
        "rest_days_home", "rest_days_away", 
        "historical_h2h_goals", "referee_card_tendency",
        
        # Advanced/External features
        "weather_impact", "crowd_atmosphere",
        "transfer_rumor_sentiment", "social_media_buzz",
        "betting_odds_implied_probability",
        "injury_risk_score", "fatigue_index",
    ])
    
    version = factory.LazyFunction(lambda: f"v{random.randint(1, 10)}.{random.randint(0, 9)}")
    ...
    description = factory.LazyAttribute(lambda obj: _generate_feature_description(obj.name))
...
class ComputedFeatureFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    entity_type = factory.LazyAttribute(
        lambda obj: obj.feature_definition.feature_type
    )
    
    entity_id = factory.LazyAttribute(
        lambda obj: _generate_entity_id(obj.entity_type)
    )
    
    gameweek = factory.LazyAttribute(
        lambda obj: random.randint(1, 38) if _needs_gameweek(obj.feature_definition.name) else None
    )
    
    season = factory.LazyFunction(lambda: generate_season_gameweek()[0])
    
    value = factory.LazyAttribute(
        lambda obj: _generate_feature_value(obj.feature_definition.name, obj.feature_definition.data_type)
    )
    
    string_value = factory.LazyAttribute(
        lambda obj: _generate_string_value(obj.feature_definition.name, obj.feature_definition.data_type)
    )
    ...
    ttl_expires_at = factory.LazyFunction(
        lambda: (datetime.now() + timedelta(hours=random.randint(24, 168))).isoformat()  # 1-7 days
    )
...
class FeatureCacheFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    feature_name = factory.fuzzy.FuzzyChoice([
        "rolling_goals_5", "xg_form_3", "team_attack_strength",
        "fixture_difficulty_weighted_form", "penalty_conversion_rate",
        "team_recent_form_5", "rest_days_home", "injury_risk_score"
    ])
    ...
    string_value = factory.LazyAttribute(
        lambda obj: _generate_string_value(obj.feature_name, "string") if random.random() < 0.2 else None
    )
    
    cached_at = factory.LazyFunction(
        lambda: (datetime.now() - timedelta(minutes=random.randint(0, 1440))).isoformat()  # Last 24 hours
    )
    
    expires_at = factory.LazyFunction(
        lambda: (datetime.now() + timedelta(hours=random.randint(1, 72))).isoformat()  # 1-72 hours
    )
...
class FeatureTimeSeriesFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    metric_name = factory.fuzzy.FuzzyChoice([
        # Player metrics
        "goals", "assists", "minutes", "points", "bonus", "saves",
        "xg", "xa", "shots", "key_passes", "tackles", "interceptions",
        "clean_sheets", "yellow_cards", "red_cards",
        
        # Team metrics
        "goals_scored", "goals_conceded", "shots_on_target", 
        "possession", "corners", "fouls", "offsides",
        
        # Fixture metrics
        "attendance", "total_goals", "total_cards", "total_corners"
    ])
    ...
    context = factory.LazyAttribute(
        lambda obj: json.dumps(_generate_timeseries_context(obj.metric_name, obj.entity_type))
    )
...
def _determine_feature_type(feature_name: str) -> str:
    ...
    if any(term in feature_name for term in ["team_", "defense", "attack"]):
        return "team"
    elif any(term in feature_name for term in ["fixture_", "h2h", "referee"]):
        return "fixture"
    elif any(term in feature_name for term in ["weather", "odds", "external"]):
        return "external"
    else:
        return "player"
...
def _determine_data_type(feature_name: str) -> str:
    ...
    if any(term in feature_name for term in ["rate", "percentage", "ratio", "strength"]):
        return "float"
    elif any(term in feature_name for term in ["indicator", "match", "derby"]):
        return "boolean"
    elif any(term in feature_name for term in ["sentiment", "tendency", "category"]):
        return "string"
    else:
...
def _generate_computation_logic(feature_name: str, feature_type: str) -> Dict[str, Any]:
    ...
    base_logic = {
        "computation_type": "rolling_average" if "rolling" in feature_name else "aggregation",
        "update_frequency": "daily",
        "lookback_window": 5 if "5" in feature_name else (3 if "3" in feature_name else 10),
    }
...
def _needs_gameweek(feature_name: str) -> bool:
    ...
    non_gameweek_features = ["penalty_conversion_rate", "historical_h2h", "team_home_advantage"]
...
def _evolve_timeseries_value(base_value: float, metric_name: str, week_offset: int) -> float:
...
def create_feature_computation_pipeline(session) -> Dict[str, List]:
    ...
    for definition in definitions:
        for _ in range(10):  # 10 computed values per feature
            computed = ComputedFeatureFactory(session=session, feature_definition=definition)
...
def create_player_feature_timeline(session, player_id: int, season: str) -> List[FeatureTimeSeries]:
