...
class MockFPLDataFetcher:
    ...
    def get_bootstrap_data(self) -> Dict[str, Any]:
        ...
        for player_id in range(1, 501):  # 500 players
            ...
            elements.append({
                ...
                "chance_of_playing_this_round": random.choice([None, 0, 25, 50, 75, 100]),
                "chance_of_playing_next_round": random.choice([None, 0, 25, 50, 75, 100]),
                "status": random.choice(["a", "i", "d", "u", "s"]),  # available, injured, doubtful, unavailable, suspended
                "news": random.choice(["", "Knock picked up in training", "Back from injury", "Expected to return soon"]),
                "news_added": datetime.now().isoformat() if random.random() < 0.3 else None,
            })
        
        # Generate element types (positions)
        element_types = [
            {"id": 1, "plural_name": "Goalkeepers", "singular_name": "Goalkeeper", "plural_name_short": "GKP"},
            {"id": 2, "plural_name": "Defenders", "singular_name": "Defender", "plural_name_short": "DEF"},
            {"id": 3, "plural_name": "Midfielders", "singular_name": "Midfielder", "plural_name_short": "MID"},
            {"id": 4, "plural_name": "Forwards", "singular_name": "Forward", "plural_name_short": "FWD"},
        ]
        ...
        for gw in range(1, 39):
            ...
            events.append({
                "id": gw,
                "name": f"Gameweek {gw}",
                "deadline_time": (event_date + timedelta(days=6, hours=11, minutes=30)).isoformat(),
                "average_entry_score": random.randint(35, 70),
                "finished": gw < self.current_gameweek,
                "data_checked": gw < self.current_gameweek,
                "highest_scoring_entry": random.randint(80, 150) if gw < self.current_gameweek else None,
                "deadline_time_epoch": int((event_date + timedelta(days=6, hours=11, minutes=30)).timestamp()),
                "deadline_time_game_offset": 0,
                "highest_score": random.randint(80, 150) if gw < self.current_gameweek else None,
                "is_previous": gw == self.current_gameweek - 1,
                "is_current": gw == self.current_gameweek,
                "is_next": gw == self.current_gameweek + 1,
                "chip_plays": {
                    "bboost": random.randint(0, 1000000) if gw < self.current_gameweek else 0,
                    "freehit": random.randint(0, 500000) if gw < self.current_gameweek else 0,
                    "wildcard": random.randint(0, 800000) if gw < self.current_gameweek else 0,
                    "3xc": random.randint(0, 300000) if gw < self.current_gameweek else 0,
                },
                "most_selected": random.randint(1, 500) if gw < self.current_gameweek else None,
                "most_transferred_in": random.randint(1, 500) if gw < self.current_gameweek else None,
                "top_element": random.randint(1, 500) if gw < self.current_gameweek else None,
                "top_element_info": {
                    "id": random.randint(1, 500),
                    "points": random.randint(10, 25)
                } if gw < self.current_gameweek else None,
                "transfers_made": random.randint(2000000, 8000000) if gw < self.current_gameweek else None,
            })
    ...
    def get_fixtures(self, gameweek: int = None) -> List[Dict[str, Any]]:
        ...
        for gw in gameweeks:
            ...
            for i in range(0, 20, 2):
                ...
                fixture = {
                    ...
                    "team_a": away_team,
                    "team_a_score": random.randint(0, 4) if gw < self.current_gameweek else None,
                    "team_h": home_team,
                    "team_h_score": random.randint(0, 4) if gw < self.current_gameweek else None,
    ...
    def get_player_data(self, player_id: int) -> Dict[str, Any]:
        ...
        for gw in range(self.current_gameweek, min(self.current_gameweek + 5, 39)):
            fixture_data = {
                ...
                "minutes": 0,
                "provisional_start_time": False,
                "kickoff_time": (datetime.now() + timedelta(days=(gw - self.current_gameweek) * 7)).isoformat(),
        ...
        for gw in range(1, self.current_gameweek):
            history_entry = {
                ...
                "kickoff_time": (datetime(2024, 8, 17) + timedelta(weeks=gw-1)).isoformat(),
    ...
    def get_fpl_team_data(self, team_id: int, gameweek: int = None) -> Dict[str, Any]:
        """Mock FPL team data."""
        if not self.logged_in:
            raise ValueError("Must be logged in to access team data")
        ...
        for i in range(15):  # 15 players in squad
            position = 1 if i < 2 else (2 if i < 7 else (3 if i < 12 else 4))
            picks.append({
                "element": random.randint(1, 500),
                "position": i + 1,
                "multiplier": 2 if i == 0 else (1 if i < 11 else 0),  # Captain gets 2x, bench gets 0
                "is_captain": i == 0,
                "is_vice_captain": i == 1,
            })
    ...
    def _generate_fixture_stats(self) -> List[Dict[str, Any]]:
        ...
        for stat_type in stat_types:
            ...
            home_players = [{"element": random.randint(1, 500), "value": random.randint(0, 3)} 
                          for _ in range(random.randint(0, 4))]
            away_players = [{"element": random.randint(1, 500), "value": random.randint(0, 3)} 
                          for _ in range(random.randint(0, 4))]
...
class MockRedisCache:
    ...
    def get(self, key: str) -> Optional[bytes]:
        ...
        if key in self._cache:
            self._stats["hits"] += 1
            return self._cache[key].encode() if isinstance(self._cache[key], str) else self._cache[key]
        else:
            self._stats["misses"] += 1
            return None
    
    def set(self, key: str, value: Union[str, bytes], ex: int = None) -> bool:
...
class MockPlayerModel:
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        proba = np.random.dirichlet(np.ones(n_classes), size=n_samples)
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        feature_names = [
            "minutes", "form", "fixture_difficulty", "team_strength",
            "xg", "xa", "shots", "key_passes", "opponent_strength", "home_advantage"
        ]
...
class MockFPLManager:
    ...
    def make_transfer(self, player_out: int, player_in: int) -> Dict[str, Any]:
        ...
        player_out_data = next((p for p in self.current_squad if p["element"] == player_out), None)
        if not player_out_data:
            raise ValueError(f"Player {player_out} not in squad")
    ...
    def set_captain(self, player_id: int) -> bool:
        ...
        # Set new captain
        captain = next((p for p in self.current_squad if p["element"] == player_id), None)
        if captain:
    ...
    def play_chip(self, chip: str) -> Dict[str, Any]:
        ...
        if chip not in self.chips_available:
            raise ValueError(f"Chip {chip} not available")
    ...
    def get_team_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "bank": self.bank,
            "team_value": sum(p["selling_price"] for p in self.current_squad) + self.bank,
...
def create_mock_fpl_fetcher(season: str = "2324", current_gameweek: int = None) -> MockFPLDataFetcher:
...
def create_mock_redis_cache(preload_data: Dict[str, Any] = None) -> MockRedisCache:
...
def create_mock_player_models(positions: List[str] = None) -> Dict[str, MockPlayerModel]:
...
def create_mock_log_scenario() -> Dict[str, Any]:
    ...
    log_scenarios = [
        {"level": "INFO", "message": "Player prediction completed", "operation": "predict", "duration_ms": 150},
        {"level": "WARNING", "message": "High fixture difficulty detected", "operation": "analyze", "duration_ms": 50},
        {"level": "ERROR", "message": "Failed to fetch player data", "operation": "fetch", "error_code": "API_ERROR"},
        {"level": "DEBUG", "message": "Cache hit for player stats", "operation": "cache", "cache_key": "player_123"},
        {"level": "INFO", "message": "Transfer optimization completed", "operation": "optimize", "duration_ms": 2500},
    ]
