...
class PlayerFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    fpl_api_id = factory.Sequence(lambda n: n + 1000)  # Start from 1000 to avoid conflicts
    name = factory.LazyFunction(lambda: get_realistic_player_name("MID"))  # Default to MID
...
class PlayerAttributesFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    return_gameweek = factory.Maybe(
        "news",
        yes_declaration=factory.fuzzy.FuzzyInteger(1, 10),
        no_declaration=None
    )
...
class PlayerAttributesExtendedFactory(PlayerAttributesFactory):
    ...
    role_confidence = factory.LazyAttribute(
        lambda obj: _calculate_role_confidence(obj)
    )
...
class HighFormPlayerAttributesFactory(PlayerAttributesExtendedFactory):
    ...
    momentum = factory.LazyAttribute(
        lambda obj: max(0.0, min(1.0, _calculate_momentum(obj.form_3_games, obj.form_10_games)))
    )
...
def _calculate_realistic_price(position: str, form_3_games: float = None) -> int:
...
def create_gameweek_players(session: Session, season: str, gameweek: int, 
                          num_players: int = 50) -> list[PlayerAttributes]:
...
def create_form_comparison_players(session: Session) -> dict:
    ...
    return {
        "high_form": [HighFormPlayerAttributesFactory(session=session) for _ in range(5)],
        "low_form": [LowFormPlayerAttributesFactory(session=session) for _ in range(5)],
        "penalty_takers": [PenaltyTakerPlayerAttributesFactory(session=session) for _ in range(3)],
        "easy_fixtures": [EasyFixturesPlayerAttributesFactory(session=session) for _ in range(5)],
        "difficult_fixtures": [DifficultFixturesPlayerAttributesFactory(session=session) for _ in range(5)],
    }


def create_historical_progression(session: Session, player: Player, start_gameweek: int = 1, 
                                num_gameweeks: int = 10, season: str = "2324") -> list[PlayerAttributes]:
