...
class DatabaseVersionFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    applied_by = factory.fuzzy.FuzzyChoice([
        "alembic_auto", "admin_user", "deployment_system", "developer_alice",
        "migration_script", "database_admin", "automated_deployment"
    ])
    
    migration_type = factory.fuzzy.FuzzyChoice([
        "initial", "upgrade", "rollback", "manual"
    ], weights=[1, 8, 2, 1])  # Mostly upgrades
    
    migration_source = factory.fuzzy.FuzzyChoice([
        "alembic", "manual", "automated"
    ], weights=[7, 2, 1])  # Mostly alembic
    ...
    migration_checksum = factory.LazyFunction(
        lambda: hashlib.md5(f"migration_content_{random.randint(10000, 99999)}".encode()).hexdigest()
    )
    ...
    ], weights=[8, 1, 1])  # Mostly validated
    
    validation_errors = factory.LazyAttribute(
        lambda obj: json.dumps(_generate_validation_errors()) if obj.validation_status == "failed" else None
    )
    
    # Performance impact tracking
    migration_duration_seconds = factory.fuzzy.FuzzyFloat(1.0, 300.0)  # 1 second to 5 minutes
    ...
    records_migrated = factory.LazyAttribute(
        lambda obj: random.randint(0, 1000000) if "data" in obj.migration_description.lower() else 0
    )
...
class MigrationHistoryFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    migration_hash = factory.LazyFunction(
        lambda: hashlib.sha256(f"migration_{random.randint(100000, 999999)}".encode()).hexdigest()[:16]
    )
    ...
    executed_at = factory.LazyFunction(
        lambda: (datetime.now() - timedelta(minutes=random.randint(0, 43200))).isoformat()  # Last 30 days
    )
    
    execution_duration_seconds = factory.fuzzy.FuzzyFloat(0.5, 120.0)  # 0.5 sec to 2 minutes
    
    executed_by = factory.fuzzy.FuzzyChoice([
        "alembic", "migration_script", "database_admin", "automated_system",
        "deployment_pipeline", "developer_local", "testing_framework"
    ])
    ...
    status = factory.fuzzy.FuzzyChoice([
        "success", "failed", "partial", "rolled_back"
    ], weights=[85, 10, 3, 2])  # Mostly successful
    
    error_message = factory.LazyAttribute(
        lambda obj: _generate_error_message() if obj.status in ["failed", "partial"] else None
    )
    ...
    rollback_sql = factory.LazyAttribute(
        lambda obj: _generate_rollback_sql(obj.migration_sql) if random.random() < 0.7 else None
    )
...
class SchemaCompatibilityFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    ], weights=[6, 2, 1, 1])  # Mostly full compatibility
    ...
    tested_combinations = factory.LazyAttribute(
        lambda obj: json.dumps(_generate_tested_combinations(obj.app_version_min, obj.app_version_max))
    )
    ...
    deprecated_at = factory.LazyAttribute(
        ...
        if obj.compatibility_level == "deprecated" else None
    )
    
    removed_at = factory.LazyAttribute(
        lambda obj: (datetime.now() + timedelta(days=random.randint(30, 180))).isoformat()
        if obj.compatibility_level == "incompatible" else None
    )
...
def _generate_rollback_info(migration_type: str) -> Dict[str, Any]:
    ...
    if migration_type == "rollback":
        return {
            ...
            "rollback_steps": ["Stop application", "Execute rollback SQL", "Restart with previous version"],
            "data_loss_risk": "low",
...
def _generate_affected_tables(description: str) -> str:
    ...
    all_tables = [
        "player", "player_attributes", "team", "fixture", "player_score",
        "model_registry", "model_version", "model_artifact", "model_performance",
        "feature_definition", "computed_feature", "feature_cache", "feature_timeseries",
        "database_version", "migration_history", "schema_compatibility"
    ]
    ...
    if "player" in description.lower():
        ...
        affected = ["model_registry", "model_version", "model_artifact", "model_performance"]
...
def _generate_tables_from_sql(sql: str) -> str:
    ...
    common_tables = ["player", "player_attributes", "team", "fixture", "model_registry", 
                    "model_version", "feature_definition", "computed_feature"]
...
def _generate_dependencies(migration_name: str) -> str:
    ...
    elif "feature" in migration_name:
...
def _generate_blocking_migrations(migration_name: str) -> str:
    ...
    elif "model_versioning" in migration_name:
...
def _generate_tested_combinations(min_version: str, max_version: str) -> List[str]:
    ...
    combinations = []
    base_versions = [min_version, max_version]
    
    # Add some intermediate versions
    for _ in range(random.randint(2, 5)):
        combinations.append(f"{min_version} -> {max_version}")
        combinations.append(f"schema_{_generate_schema_version(min_version)} + app_{max_version}")
...
def _generate_known_issues(compatibility_level: str) -> List[str]:
    ...
    num_issues = {"limited": 1, "deprecated": 2, "incompatible": 3}.get(compatibility_level, 0)
...
def create_migration_timeline(session, start_version: str = "1.0.0", num_versions: int = 10) -> List[DatabaseVersion]:
...
def create_failed_migration_scenario(session) -> Dict[str, Any]:
    ...
    failed_history = FailedMigrationHistoryFactory(
        session=session,
        database_version=failed_version,
