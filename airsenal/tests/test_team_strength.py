...
from airsenal.framework.team_strength import (
    ...
    update_team_strengths_for_gameweek,
    validate_team_strength_model,
)
...
class TestTeamStrengthModels(unittest.TestCase):
    ...
    def test_team_strength_model_creation(self):
        ...
        self.assertEqual(retrieved.team, "ARS")
        self.assertEqual(retrieved.season, "2023-24")
        self.assertEqual(retrieved.gameweek, 5)
    ...
    def test_team_strength_history_model(self):
        ...
        retrieved = self.session.query(TeamStrengthHistory).filter_by(team="LIV").first()
        self.assertIsNotNone(retrieved)
        self.assertEqual(retrieved.gameweek, 3)
    ...
    def test_model_string_representations(self):
        ...
        self.assertEqual(str(strength), expected_str)
        ...
        self.assertIn("TeamStrengthHistory", str(history))
        self.assertIn("CHE", str(history))
...
class TestTeamStrengthAnalyzer(unittest.TestCase):
    ...
    def test_analyzer_initialization(self):
        ...
        self.assertEqual(self.analyzer.alpha, 0.2)
        self.assertEqual(self.analyzer.mcmc_samples, 100)
        self.assertEqual(self.analyzer.mcmc_warmup, 50)
        self.assertEqual(self.analyzer.model_version, "1.0.0")
    ...
    def test_match_data_preparation(self):
        ...
        # Verify team mapping
        self.assertIn("team_to_idx", team_mapping)
        self.assertIn("idx_to_team", team_mapping)
        self.assertEqual(team_mapping["n_teams"], 4)  # ARS, CHE, LIV, MCI
        
        # Verify arrays
        self.assertEqual(len(home_teams), 3)
        self.assertEqual(len(away_teams), 3)
        self.assertEqual(len(home_goals), 3)
        self.assertEqual(len(away_goals), 3)
    ...
    def test_attacking_strength_calculation(self):
        ...
        self.assertIn("attacking_strength", attacking_metrics)
        self.assertIn("goals_per_game", attacking_metrics)
        self.assertIn("expected_goals_per_game", attacking_metrics)
        self.assertIn("data_quality", attacking_metrics)
        
        # Verify reasonable values
        self.assertGreaterEqual(attacking_metrics["data_quality"], 0.0)
        self.assertLessEqual(attacking_metrics["data_quality"], 1.0)
    ...
    def test_defensive_strength_calculation(self):
        ...
        # Verify return structure
        self.assertIn("defensive_strength", defensive_metrics)
        self.assertIn("goals_conceded_per_game", defensive_metrics)
        self.assertIn("clean_sheet_probability", defensive_metrics)
        self.assertIn("data_quality", defensive_metrics)
    ...
    def test_form_metrics_calculation(self):
        ...
        self.assertIn("form_weighted_strength", form_metrics)
        self.assertIn("recent_performance_trend", form_metrics)
        self.assertIn("momentum_factor", form_metrics)
        
        # Verify reasonable ranges
        self.assertGreaterEqual(form_metrics["form_weighted_strength"], 0.0)
        self.assertLessEqual(form_metrics["form_weighted_strength"], 1.0)
        self.assertGreaterEqual(form_metrics["momentum_factor"], -1.0)
        self.assertLessEqual(form_metrics["momentum_factor"], 1.0)
    ...
    def test_composite_strength_calculations(self):
        ...
        self.assertIsInstance(attacking_strength, float)
        self.assertGreaterEqual(attacking_strength, -1.0)
        self.assertLessEqual(attacking_strength, 1.0)
        ...
        self.assertIsInstance(defensive_strength, float)
        self.assertGreaterEqual(defensive_strength, -1.0)
        self.assertLessEqual(defensive_strength, 1.0)
    ...
    def test_default_strength_values(self):
        ...
        # Verify structure
        self.assertIn("attacking_strength", default_attacking)
        self.assertIn("data_quality", default_attacking)
        self.assertEqual(default_attacking["data_quality"], 0.0)
        
        self.assertIn("defensive_strength", default_defensive)
        self.assertIn("data_quality", default_defensive)
        self.assertEqual(default_defensive["data_quality"], 0.0)
...
class TestTeamStrengthAPI(unittest.TestCase):
    ...
    def test_get_team_strength_endpoint(self, mock_get_strength):
        ...
        self.assertEqual(data["team"], "ARS")
        self.assertIn("attacking_strength", data)
        self.assertIn("defensive_strength", data)
    ...
    def test_compare_team_strengths_endpoint(self, mock_compare):
        ...
        self.assertEqual(data["head_to_head"]["predicted_winner"], "ARS")
    ...
    def test_team_strength_trends_endpoint(self, mock_get_trends):
        ...
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data["team"], "MCI")
    ...
    def test_update_team_strengths_endpoint(self, mock_update):
        ...
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data["teams_updated"], 20)
    ...
    def test_validate_model_endpoint(self, mock_validate):
        ...
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertGreater(data["pearson_correlation"], 0.7)
        self.assertTrue(data["correlation_meets_target"])
    ...
    def test_invalid_gameweek_parameter(self):
        ...
        self.assertIn("Invalid gameweek", data["error"])
...
class TestTeamStrengthValidation(unittest.TestCase):
    ...
    def test_strength_prediction_validation(self):
        ...
        self.session.query.return_value.join.return_value.filter.return_value.order_by.return_value.__iter__ = MagicMock(return_value=iter(mock_matches))
        
        # Mock team strengths
        mock_strength_ars = MagicMock(overall_strength_home=1.2, overall_strength_away=0.9)
        mock_strength_che = MagicMock(overall_strength_home=1.0, overall_strength_away=0.8)
        mock_strength_liv = MagicMock(overall_strength_home=1.1, overall_strength_away=1.0)
        mock_strength_mci = MagicMock(overall_strength_home=1.3, overall_strength_away=1.2)
        ...
        self.assertIn("pearson_correlation", validation_results)
        self.assertIn("prediction_accuracy", validation_results)
        self.assertIn("mean_absolute_error", validation_results)
        self.assertIn("correlation_meets_target", validation_results)
        
        # Verify reasonable values
        self.assertIsInstance(validation_results["pearson_correlation"], float)
        self.assertIsInstance(validation_results["prediction_accuracy"], float)
    ...
    def test_correlation_target_compliance(self):
        ...
        with patch('scipy.stats.pearsonr') as mock_pearsonr:
            ...
            self.session.query.return_value.join.return_value.filter.return_value.order_by.return_value.__iter__ = MagicMock(return_value=iter([]))
            
            validation_results = self.analyzer.validate_strength_predictions("2023-24")
            
            # Verify correlation exceeds target
            self.assertGreater(validation_results["pearson_correlation"], 0.7)
            self.assertTrue(validation_results["correlation_meets_target"])
...
class TestTeamStrengthIntegration(unittest.TestCase):
    ...
    def test_end_to_end_strength_calculation(self):
        ...
        try:
            ...
            for team, strength_data in strengths.items():
                self.assertIn("attacking_strength_home", strength_data)
                self.assertIn("defensive_strength_home", strength_data)
                self.assertIn("overall_strength_home", strength_data)
                self.assertIn("model_version", strength_data)
    ...
    def test_database_integration(self):
        ...
        self.assertIsNotNone(saved_strength.id)
        
        # Verify retrieval
        retrieved_strength = analyzer.get_team_strength("ARS", "2023-24", 3)
        self.assertIsNotNone(retrieved_strength)
        self.assertEqual(retrieved_strength.team, "ARS")
        self.assertAlmostEqual(retrieved_strength.attacking_strength_home, 1.1, places=2)
...
class TestFactoryFunctions(unittest.TestCase):
    ...
    def test_create_team_strength_analyzer(self):
        ...
        analyzer = create_team_strength_analyzer(
            mock_session,
            exponential_smoothing_alpha=0.25,
            mcmc_samples=1500
        )
        
        self.assertIsInstance(analyzer, TeamStrengthAnalyzer)
        self.assertEqual(analyzer.alpha, 0.25)
        self.assertEqual(analyzer.mcmc_samples, 1500)
    ...
    def test_api_helper_functions(self, mock_create_analyzer):
        ...
        result = get_team_strength_for_api("ARS", "2023-24", 5, MagicMock())
