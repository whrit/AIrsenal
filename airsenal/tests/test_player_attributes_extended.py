...
def test_extended_player_attributes_creation(db_session, sample_player):
    ...
    retrieved = (
        db_session.query(PlayerAttributes)
        .filter_by(player_id=sample_player.player_id)
        .first()
    )
...
def test_query_performance_scenarios(db_session, sample_player):
    ...
    high_form_players = (
        db_session.query(PlayerAttributes)
        .filter(PlayerAttributes.form_3_games >= 7.0)
        .all()
    )
    ...
    high_xg_players = (
        db_session.query(PlayerAttributes)
        .filter(PlayerAttributes.xg_per_90 >= 1.0)
        .all()
    )
...
def test_relationship_integrity(db_session, sample_player):
    ...
    player = (
        db_session.query(Player).filter_by(player_id=sample_player.player_id).first()
    )
...
def test_multiple_gameweeks_form_progression(db_session, sample_player):
    ...
    excellent_form = (
        db_session.query(PlayerAttributes)
        .filter(PlayerAttributes.form_3_games >= 8.0, PlayerAttributes.momentum > 0)
        .all()
    )
...
def test_fixture_difficulty_scenarios(db_session, sample_player):
    ...
    easy_fixtures = (
        db_session.query(PlayerAttributes)
        .filter(PlayerAttributes.next_3_fixture_difficulty <= 2.5)
        .all()
    )
    ...
    difficult_fixtures = (
        db_session.query(PlayerAttributes)
        .filter(PlayerAttributes.next_3_fixture_difficulty >= 4.0)
        .all()
    )
