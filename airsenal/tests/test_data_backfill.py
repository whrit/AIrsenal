...
from airsenal.framework.schema import (
    Fixture, Player, PlayerScore, session_scope
)
from airsenal.framework.xg_data_provider import XGDataPoint, XGDataManager
...
class TestBackfillValidator:
    ...
    def test_xg_data_consistency_validation(self, mock_session_scope):
        ...
        mock_session.query.return_value.filter.return_value.filter.return_value.all.return_value = [mock_score]
    ...
    def test_missing_data_detection(self, mock_session_scope):
        ...
        mock_session.query.return_value.filter.return_value.count.return_value = 100  # total scores
        mock_session.query.return_value.filter.return_value.filter.return_value.count.return_value = 30  # with xG
        
        # Mock gameweeks
        mock_session.query.return_value.filter.return_value.distinct.return_value.all.return_value = [(1,), (2,), (3,)]
    ...
    def test_statistical_outlier_detection(self, mock_session_scope):
        ...
        mock_session.query.return_value.filter.return_value.filter.return_value.all.return_value = [mock_score]
        mock_session.query.return_value.filter.return_value.first.return_value = MagicMock(count_xg=100)
...
class TestBackfillResult:
    ...
    def test_issues_by_severity_grouping(self):
        ...
        result.add_validation_issue(ValidationSeverity.ERROR, "test", "Error 1", "entity")
        result.add_validation_issue(ValidationSeverity.ERROR, "test", "Error 2", "entity")
        result.add_validation_issue(ValidationSeverity.WARNING, "test", "Warning 1", "entity")
...
class TestHistoricalDataBackfiller:
    ...
    def test_backfill_single_gameweek_success(self, mock_tracker_class, mock_session_scope):
        """Test successful single gameweek backfill"""
        # Mock XG manager
        mock_xg_manager = MagicMock()
        mock_xg_manager.sync_xg_data_for_gameweek.return_value = 50  # 50 records updated
    ...
    @patch('airsenal.framework.data_backfill.session_scope')
    @patch('airsenal.framework.data_backfill.BackfillProgressTracker')
    def test_backfill_single_gameweek_no_data(self, mock_tracker_class, mock_session_scope):
    ...
    @patch('airsenal.framework.data_backfill.session_scope')
    @patch('airsenal.framework.data_backfill.BackfillProgressTracker')
    def test_backfill_single_gameweek_failure(self, mock_tracker_class, mock_session_scope):
    ...
    def test_backfill_season_no_fixtures(self, mock_tracker_class, mock_session_scope):
        ...
        assert result.status == BackfillStatus.FAILED
        assert result.has_critical_issues
        assert any("No fixtures found" in issue.description for issue in result.validation_issues)
...
class TestIntegration:
    ...
    def test_end_to_end_backfill_workflow(self):
        ...
        with patch('airsenal.framework.data_backfill.session_scope'):
            with patch.object(backfiller.validator, 'validate_backfilled_data') as mock_validate:
    ...
    def test_parallel_processing_performance(self):
        ...
        with patch('airsenal.framework.data_backfill.session_scope'):
            with patch.object(backfiller.validator, 'validate_backfilled_data', return_value=[]):
                with patch.object(backfiller, '_generate_reconciliation_report', return_value={}):
...
class TestErrorHandling:
    ...
    def test_api_rate_limit_handling(self):
        ...
        mock_xg_manager = MagicMock()
        mock_xg_manager.sync_xg_data_for_gameweek.side_effect = Exception("Rate limit exceeded")
